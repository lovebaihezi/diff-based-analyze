...
pub fn initWithContent(name: []const u8, buffer: []const u8) LLVMContentError!Module {
    ...
    module.ctx = llvm.LLVMContextCreate();
    var out_msg: [*c]u8 = 0x0;
    if (llvm.LLVMCreateMemoryBufferWithMemoryRange(buffer.ptr, buffer.len, name.ptr, 0) != 0) {
        if (out_msg != 0x0) {
            std.log.err("failed to read bitcode from stdin, output message: {s}", .{out_msg});
        }
        return error.create_mem_buf_with_content_failed;
    }
    return module;
}

pub fn parse_bite_code_functions(module: *Module) void {
    if (llvm.LLVMParseBitcode2(module.mem_buf, &module.mod_ref) != 0) {
        std.log.err("failed to read bitcode from stdin, output message: {s}", .{module.out_msg});
