 ...
 *
 * Logger must be obtained from create_or_get_logger(), therefore constructors
 * are private
 */
...
public:
   ...
   * One spsc queue per caller thread. This function is enabled only when all
   * arguments are fundamental types. This is the fastest way possible to log
  ...
  log_statement(QUILL_MAYBE_UNUSED LogLevel dynamic_log_level,
    ...
    // Store the timestamp of the log statement at the start of the call. This
    // gives more accurate timestamp especially if the queue is full This is
    // very rare but might lead to out of order timestamp in the log file if we
    // block on push for too long
    ...
    if (QUILL_UNLIKELY(thread_context == nullptr)) {
      // This caches the ThreadContext pointer to avoid repeatedly calling
    ...
    if constexpr (has_dynamic_log_level) {
      // For the dynamic log level we want to add to the total size to store the
      // dynamic log level
    ...
    if constexpr (frontend_options_t::queue_type ==
                  QueueType::UnboundedUnlimited) {
      assert(write_buffer && "Unbounded unlimited queue will always allocate "
                             "and have enough capacity");
    } else if constexpr ((frontend_options_t::queue_type ==
                          QueueType::BoundedDropping) ||
                         (frontend_options_t::queue_type ==
      ...
      if (QUILL_UNLIKELY(write_buffer == nullptr)) {
        ...
        do {
          if constexpr (frontend_options_t::blocking_queue_retry_interval_ns >
                        0) {
            std::this_thread::sleep_for(std::chrono::nanoseconds{
                frontend_options_t::blocking_queue_retry_interval_ns});
          }
    ...
    write_buffer = _encode_header(
        write_buffer, current_timestamp, macro_metadata, this,
        detail::decode_and_store_args<detail::remove_cvref_t<Args>...>);

    // encode remaining arguments
    detail::encode(write_buffer,
                   thread_context->get_conditional_arg_size_cache(),
                   fmt_args...);

    if constexpr (has_dynamic_log_level) {
      // write the dynamic log level
      // The reason we write it last is that is less likely to break the
      // alignment in the buffer
    ...
    assert(total_size == (static_cast<size_t>(write_buffer - write_begin)) &&
           "The committed write bytes must be equal to the total_size "
           "requested bytes");
#endif

    thread_context->get_spsc_queue<frontend_options_t::queue_type>()
        .finish_and_commit_write(total_size);
   ...
   * Stores messages logged with LOG_BACKTRACE in a ring buffer messages and
   * displays them later on demand.
   * @param max_capacity The max number of messages to store in the backtrace
   * @param flush_level If this loggers logs any message higher or equal to this
   * severity level the backtrace will also get flushed. Default level is None
   * meaning the user has to call flush_backtrace explicitly
   */
  void init_backtrace(uint32_t max_capacity,
                      LogLevel flush_level = LogLevel::None) {
    // we do not care about the other fields, except
  ...
  void flush_backtrace() {
    ...
    // We do not want to drop the message if a dropping queue is used
    while (
        !this->log_statement<false, false>(LogLevel::None, &macro_metadata)) {
      std::this_thread::sleep_for(std::chrono::nanoseconds{100});
    }
  }

  /**
   * Blocks the calling thread until all log messages up to the current
   * timestamp are flushed.
   *
   * The backend thread will invoke the write operation on all sinks for all
   * loggers up to the point (timestamp) when this function is invoked.
   *
   * @param sleep_duration_ns The duration in nanoseconds to sleep between
   * retries when the blocking queue is full, and between checks for the flush
   * completion. Default is 100 nanoseconds.
   *
   * @note This function should only be called when the backend worker is
   * running after Backend::start(...)
   * @note This function will block the calling thread until the flush message
   * is processed by the backend thread. The calling thread can block for up to
   * backend_options.sleep_duration. If you configure a custom long sleep
   * duration on the backend thread, e.g., backend_options.sleep_duration =
   * std::chrono::minutes{1}, then you should ideally avoid calling this
   * function as you can block for long period of times unless you use another
   * thread that calls Backend::notify()
   */
  void flush_log(uint32_t sleep_duration_ns = 100) {
    ...
    while (!this->log_statement<false, false>(
        LogLevel::None, &macro_metadata,
        reinterpret_cast<uintptr_t>(backend_thread_flushed_ptr))) {
      if (sleep_duration_ns > 0) {
        std::this_thread::sleep_for(
            std::chrono::nanoseconds{sleep_duration_ns});
      } else {
        std::this_thread::yield();
      }
    }

    // The caller thread keeps checking the flag until the backend thread
    // flushes
...
private:
  ...
  LoggerImpl(std::string logger_name, std::vector<std::shared_ptr<Sink>> sinks,
             PatternFormatterOptions pattern_formatter_options,
   ...
   *
   * This function helps the compiler to better optimize the operation using
   * SSE/AVX instructions. Avoid using `std::byte*& write_buffer` as parameter
   * as it may prevent these optimizations.
   *
   * @return Updated pointer to the write buffer after encoding the header.
   */
  QUILL_NODISCARD QUILL_ATTRIBUTE_HOT static std::byte *
  _encode_header(std::byte *write_buffer, uint64_t timestamp,
                 MacroMetadata const *metadata, detail::LoggerBase *logger_ctx,
  ...
  _prepare_write_buffer(size_t total_size) {
    constexpr bool is_unbounded_queue =
    ...
    if constexpr (is_unbounded_queue) {
      // MSVC doesn't like the template keyword, but every other compiler
      // requires it
#if defined(_MSC_VER)
      return thread_context->get_spsc_queue<frontend_options_t::queue_type>()
          .prepare_write(total_size, frontend_options_t::queue_type);
#else
      return thread_context->get_spsc_queue<frontend_options_t::queue_type>()
