 ...
 *
 * @remarks If the input string is empty or the conversion fails, an empty
 * string is returned.
 */
inline std::string utf8_encode(std::byte const *data, size_t wide_str_len) {
  ...
  // Because we are using a std::byte* buffer and the data are coming from there
  // we will cast back the data to char* and then copy them to a wide string
  // buffer before accessing them
  ...
  int const size_needed = WideCharToMultiByte(CP_UTF8, 0, wide_buffer.get(),
                                              static_cast<int>(wide_str_len),
  ...
  if (WideCharToMultiByte(CP_UTF8, 0, wide_buffer.get(),
                          static_cast<int>(wide_str_len), &ret_val[0],
...
inline std::string utf8_encode(std::wstring_view str) {
  return utf8_encode(reinterpret_cast<std::byte const *>(str.data()),
                     str.size());
}
} // namespace detail
