...
public:
  /***/
  explicit PatternFormatterOptions(
   ...
   *
   * It can include various placeholders that will be replaced with actual
   * values when formatting the log message.
   *
   * %(time)                    - Human-readable timestamp representing when the
   * log statement was created.
   * %(file_name)               - Name of the source file where the logging call
   * was issued.
   * %(full_path)               - Full path of the source file where the logging
   * call was issued.
   * %(caller_function)         - Name of the function containing the logging
   * call.
   * %(log_level)               - Textual representation of the logging level
   * for the message.
   * %(log_level_short_code)    - Abbreviated log level name.
   * %(line_number)             - Line number in the source file where the
   * logging call was issued.
   * %(logger)                  - Name of the logger used to log the call.
   * %(message)                 - The logged message itself.
   * %(thread_id)               - ID of the thread in which the logging call was
   * made.
   * %(thread_name)             - Name of the thread. Must be set before the
   * first log statement on that thread.
   * %(process_id)              - ID of the process in which the logging call
   * was made.
   * %(source_location)         - Full source file path and line number as a
   * single string.
   * %(short_source_location)   - Shortened source file name and line number as
   * a single string.
   * %(tags)                    - Additional custom tags appended to the message
   * when _TAGS macros are used.
   * %(named_args)              - Key-value pairs appended to the message. Only
   * applicable when the message has named args; remains empty otherwise.
   *
   * @warning The same attribute cannot be used twice in the same format
   * pattern.
   */
  std::string format_pattern;
  ...
  bool operator==(const PatternFormatterOptions &other) const {
    return format_pattern == other.format_pattern &&
           timestamp_pattern == other.timestamp_pattern &&
           timestamp_timezone == other.timestamp_timezone &&
           add_metadata_to_multi_line_logs ==
               other.add_metadata_to_multi_line_logs;
  }
