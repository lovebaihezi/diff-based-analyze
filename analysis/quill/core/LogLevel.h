...
enum class LogLevel : uint8_t {
  ...
  Error,
  Critical,
  Backtrace, /**< This is only used for backtrace logging. Should not be set by
                the user. */
  None,
  Dynamic /**< This is only used for dynamic logging. Should not be set by the
             user. */
};

namespace detail {
QUILL_NODISCARD QUILL_ATTRIBUTE_HOT inline std::string_view
log_level_to_string(LogLevel log_level, std::string const *log_levels_strings,
                    size_t log_levels_strings_size) {
  auto const log_lvl = static_cast<uint32_t>(log_level);

  if (QUILL_UNLIKELY(log_lvl >=
                     static_cast<uint32_t>(log_levels_strings_size))) {
    std::string const error_msg =
        "Invalid get_log_level value \"" + std::to_string(log_lvl) + "\"";
...
QUILL_NODISCARD LogLevel inline loglevel_from_string(std::string log_level) {
  ...
  std::string const error_msg =
      "LogLevel enum value does not exist for \"" + log_level + "\"";
  QUILL_THROW(QuillError{error_msg});
}
