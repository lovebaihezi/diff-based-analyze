...
public:
  ...
  QUILL_NODISCARD QUILL_ATTRIBUTE_HOT
      std::conditional_t<(queue_type == QueueType::UnboundedBlocking) ||
                             (queue_type == QueueType::UnboundedUnlimited) ||
  ...
  /***/
  QUILL_NODISCARD QUILL_ATTRIBUTE_HOT SizeCacheVector &
  get_conditional_arg_size_cache() noexcept {
    return _conditional_arg_size_cache;
  }

  /***/
  QUILL_NODISCARD QUILL_ATTRIBUTE_HOT bool
  has_bounded_queue_type() const noexcept {
    return (_queue_type == QueueType::BoundedBlocking) ||
           (_queue_type == QueueType::BoundedDropping);
  }

  /***/
  QUILL_NODISCARD QUILL_ATTRIBUTE_HOT bool
  has_unbounded_queue_type() const noexcept {
    return (_queue_type == QueueType::UnboundedBlocking) ||
           (_queue_type == QueueType::UnboundedDropping) ||
           (_queue_type == QueueType::UnboundedUnlimited);
  }

  /***/
  QUILL_NODISCARD QUILL_ATTRIBUTE_HOT bool has_dropping_queue() const noexcept {
    return (_queue_type == QueueType::UnboundedDropping) ||
           (_queue_type == QueueType::BoundedDropping);
  }
  ...
  QUILL_NODISCARD QUILL_ATTRIBUTE_HOT bool has_blocking_queue() const noexcept {
    return (_queue_type == QueueType::UnboundedBlocking) ||
           (_queue_type == QueueType::BoundedBlocking);
  }

  /***/
  QUILL_NODISCARD QUILL_ATTRIBUTE_HOT SpscQueueUnion const &
  get_spsc_queue_union() const noexcept {
    return _spsc_queue_union;
  }

  /***/
  QUILL_NODISCARD QUILL_ATTRIBUTE_HOT SpscQueueUnion &
  get_spsc_queue_union() noexcept {
    return _spsc_queue_union;
  }
  ...
  /***/
  QUILL_NODISCARD QUILL_ATTRIBUTE_HOT size_t
  get_and_reset_failure_counter() noexcept {
...
private:
  ...
  SizeCacheVector
      _conditional_arg_size_cache; /**< cache for storing sizes needed for
                                      specific operations, such as when calling
                                      `strn` functions or when a loop is
                                      required e.g. caching the size of a type
                                    */
  std::string _thread_id =
      std::to_string(get_thread_id());          /**< cached thread pid */
  std::string _thread_name = get_thread_name(); /**< cached thread name */
  std::shared_ptr<TransitEventBuffer>
      _transit_event_buffer; /**< backend thread buffer. this could be
                                unique_ptr but it is shared_ptr because of the
                                forward declaration */
  QueueType _queue_type;
  std::atomic<bool> _valid{true}; /**< is this context valid, set by the
                                     frontend, read by the backend thread */
...
public:
  ...
  /***/
  template <typename TCallback> void for_each_thread_context(TCallback cb) {
  ...
  /***/
  void register_thread_context(
      std::shared_ptr<ThreadContext> const &thread_context) {
    _spinlock.lock();
  ...
  /***/
  QUILL_NODISCARD QUILL_ATTRIBUTE_HOT bool
  has_invalid_thread_context() const noexcept {
    // Here we do relaxed because if the value is not zero we will look inside
    // ThreadContext invalid flag that is also a relaxed atomic, and then we
    // will look into the SPSC queue size that is also atomic Even if we don't
  ...
  QUILL_NODISCARD QUILL_ATTRIBUTE_HOT bool new_thread_context_flag() noexcept {
    // Again relaxed memory model as in case it is false we will acquire the
    // lock
    if (_new_thread_context_flag.load(std::memory_order_relaxed)) {
      // if the variable was updated to true, set it to false,
      // There should not be any race condition here as this is the only place
      // _changed is set to false, and we will return true anyway
  ...
  /***/
  void remove_shared_invalidated_thread_context(
      ThreadContext const *thread_context) {
    LockGuard const lock{_spinlock};

    // We could use std::find_if, but since this header is included in Logger.h,
    // which is essential for logging purposes, we aim to minimize the number of
    // includes in that path. Therefore, we implement our own find_if loop here.
    auto thread_context_it = _thread_contexts.end();
    for (auto it = _thread_contexts.begin(); it != _thread_contexts.end();
         ++it) {
...
private:
  std::vector<std::shared_ptr<ThreadContext>>
      _thread_contexts; /**< The registered contexts */
  Spinlock _spinlock;   /**< Protect access when register contexts or removing
                           contexts */
...
public:
  /***/
  ScopedThreadContext(QueueType queue_type, uint32_t spsc_queue_capacity,
                      bool huge_pages_enabled)
      : _thread_context(std::make_shared<ThreadContext>(
            queue_type, spsc_queue_capacity, huge_pages_enabled)) {
#ifndef NDEBUG
    // Thread-local flag to track if an instance has been created for this
    // thread. This ensures that get_local_thread_context() is not called with
    // different template arguments when using custom FrontendOptions. Having
    // multiple thread contexts in a single thread is fine and functional but
    // goes against the design principle of maintaining a single thread context
    // per thread.
    thread_local bool thread_local_instance_created = false;

    assert(
        !thread_local_instance_created &&
  ...
  ~ScopedThreadContext() noexcept {
    // This destructor will get called when the thread that created this wrapper
    // stops we will only invalidate the thread context The backend thread will
    // empty an invalidated ThreadContext and then remove_file it from the
    // ThreadContextCollection There is only exception for the thread who owns
    // the ThreadContextCollection the main thread. The thread context of the
    // main thread can get deleted before getting invalidated
...
private:
   ...
   * ThreadContextCollection can be destructed last even after the logger
   * singleton destruction so we use shared_ptr */
...
get_local_thread_context() noexcept {
  thread_local ScopedThreadContext scoped_thread_context{
      ...
      TFrontendOptions::huge_pages_enabled};
