...
public:
  ...
  QUILL_ATTRIBUTE_HOT value_type push_back(value_type value) {
    if (_size == _capacity) {
      ...
      if (QUILL_UNLIKELY(new_capacity <= _capacity)) {
        QUILL_THROW(QuillError{"This unreachable code is here only to suppress "
   ...
   * Access element
   */
  QUILL_NODISCARD QUILL_ATTRIBUTE_HOT value_type
...
/**
 * A vector that stores sizes for specific operations using `uint32_t` to
 * optimize space. The capacity of 12 is chosen to fit within a full cache line
