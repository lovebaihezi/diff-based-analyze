...
private:
  struct SinkInfo {
    ...
    SinkInfo(std::string sid, std::weak_ptr<Sink> sptr)
        : sink_id(static_cast<std::string &&>(sid)),
          sink_ptr(static_cast<std::weak_ptr<Sink> &&>(sptr)) {};
...
public:
  ...
  /***/
  QUILL_NODISCARD std::shared_ptr<Sink>
  get_sink(std::string const &sink_name) const {
    // The sinks are used by the backend thread, so after their creation we want
    // to avoid mutating their member variables.
    ...
    if (QUILL_UNLIKELY(!sink)) {
      QUILL_THROW(
          QuillError{"Sink with name \"" + sink_name + "\" does not exist"});
    }
  ...
  std::shared_ptr<Sink> create_or_get_sink(std::string const &sink_name,
                                           Args &&...args) {
    // The sinks are used by the backend thread, so after their creation we want
    // to avoid mutating their member variables.
    LockGuard const lock{_spinlock};
    ...
    if (!sink) {
      if constexpr (std::disjunction_v<std::is_same<FileSink, TSink>,
                                       std::is_base_of<FileSink, TSink>>) {
        sink =
            std::make_shared<TSink>(sink_name, static_cast<Args &&>(args)...);
      } else {
  ...
  uint32_t cleanup_unused_sinks() {
    // this needs to take a lock each time. The backend logging thread should be
    // carefully call it only when needed
    LockGuard const lock{_spinlock};
...
private:
  ...
  /***/
  void _insert_sink(std::string const &sink_name,
                    std::shared_ptr<Sink> const &sink) {
    auto search_it =
        std::lower_bound(_sinks.begin(), _sinks.end(), sink_name,
                         [](SinkInfo const &elem, std::string const &b) {
  ...
  /***/
  QUILL_NODISCARD std::shared_ptr<Sink>
  _find_sink(std::string const &target) const noexcept {
