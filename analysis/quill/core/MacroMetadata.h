...
class MacroMetadata {
public:
  enum Event : uint8_t { Log, InitBacktrace, FlushBacktrace, Flush };

  constexpr MacroMetadata(char const *source_location,
                          char const *caller_function,
                          char const *message_format, char const *tags,
                          LogLevel log_level, Event event) noexcept
      : _source_location(source_location), _caller_function(caller_function),
        _message_format(message_format), _tags(tags),
        _colon_separator_pos(_calc_colon_separator_pos()),
        _file_name_pos(_calc_file_name_pos()), _log_level(log_level),
        _event(event) {
  ...
  QUILL_NODISCARD std::string_view file_name() const noexcept {
    return std::string_view{
        _source_location + _file_name_pos,
...
private:
  ...
  /***/
  QUILL_NODISCARD constexpr uint16_t
  _calc_colon_separator_pos() const noexcept {
  ...
  static constexpr bool _contains_named_args(std::string_view fmt) noexcept {
    ...
    // Iterates the format string and checks if any characters are contained
    // inside `{}`
