 ...
 */
template <typename T> class BoundedSPSCQueueImpl {
public:
  using integer_type = T;

  QUILL_ATTRIBUTE_HOT explicit BoundedSPSCQueueImpl(
      integer_type capacity, bool huges_pages_enabled = false,
      integer_type reader_store_percent = 5)
      : _capacity(next_power_of_two(capacity)), _mask(_capacity - 1),
        _bytes_per_batch(static_cast<integer_type>(
            _capacity * static_cast<double>(reader_store_percent) / 100.0)),
        _storage(static_cast<std::byte *>(
            _alloc_aligned(2ull * static_cast<uint64_t>(_capacity),
    ...
    for (uint64_t i = 0; i < (2ull * static_cast<uint64_t>(_capacity));
         i += CACHE_LINE_SIZE) {
    ...
    for (uint64_t i = 0; i < cache_lines; ++i) {
      _mm_prefetch(
          reinterpret_cast<char const *>(_storage + (CACHE_LINE_SIZE * i)),
          _MM_HINT_T0);
    }
#endif
  }
  ...
  prepare_write(integer_type n) noexcept {
    if ((_capacity -
         static_cast<integer_type>(_writer_pos - _reader_pos_cache)) < n) {
  ...
  QUILL_ATTRIBUTE_HOT void commit_write() noexcept {
    ...
    _mm_prefetch(reinterpret_cast<char const *>(
                     _storage + (_writer_pos & _mask) + (CACHE_LINE_SIZE * 10)),
                 _MM_HINT_T0);
#endif
  }
  ...
  QUILL_ATTRIBUTE_HOT void commit_read() noexcept {
    if (static_cast<integer_type>(
            _reader_pos - _atomic_reader_pos.load(std::memory_order_relaxed)) >=
        _bytes_per_batch) {
  ...
  QUILL_NODISCARD QUILL_ATTRIBUTE_HOT bool empty() const noexcept {
    if (_writer_pos_cache == _reader_pos) {
      // if we think the queue is empty we also load the atomic variable to
      // check further
  ...
  QUILL_NODISCARD bool huge_pages_enabled() const noexcept {
    return _huge_pages_enabled;
  }

private:
#if defined(QUILL_X86ARCH)
  QUILL_ATTRIBUTE_HOT void _flush_cachelines(integer_type &last,
                                             integer_type offset) {
   ...
   */
  QUILL_NODISCARD static std::byte *_align_pointer(void *pointer,
                                                   size_t alignment) noexcept {
    assert(is_power_of_two(alignment) && "alignment must be a power of two");
    return reinterpret_cast<std::byte *>(
        (reinterpret_cast<uintptr_t>(pointer) + (alignment - 1ul)) &
        ~(alignment - 1ul));
  }
   ...
   * @param alignment specifies the alignment. Must be a valid alignment
  ...
  QUILL_NODISCARD static void *
  _alloc_aligned(size_t size, size_t alignment,
                 QUILL_MAYBE_UNUSED bool huges_pages_enabled) {
#if defined(_WIN32)
    void *p = _aligned_malloc(size, alignment);

    if (!p) {
      QUILL_THROW(QuillError{
          std::string{"_alloc_aligned failed with error message errno: "} +
          std::to_string(errno)});
    }
    ...
    void *mem =
        ::mmap(nullptr, total_size, PROT_READ | PROT_WRITE, flags, -1, 0);

    if (mem == MAP_FAILED) {
      QUILL_THROW(QuillError{std::string{"mmap failed. errno: "} +
                             std::to_string(errno) +
                             " error: " + strerror(errno)});
    }

    // Calculate the aligned address after the metadata
    std::byte *aligned_address = _align_pointer(
        static_cast<std::byte *>(mem) + metadata_size, alignment);

    // Calculate the offset from the original memory location
    auto const offset =
        static_cast<size_t>(aligned_address - static_cast<std::byte *>(mem));

    // Store the size and offset information in the metadata
    std::memcpy(aligned_address - sizeof(size_t), &total_size,
                sizeof(total_size));
    std::memcpy(aligned_address - (2u * sizeof(size_t)), &offset,
                sizeof(offset));
  ...
  void static _free_aligned(void *ptr) noexcept {
    ...
    size_t offset;
    std::memcpy(&offset, static_cast<std::byte *>(ptr) - (2u * sizeof(size_t)),
                sizeof(offset));

    size_t total_size;
    std::memcpy(&total_size, static_cast<std::byte *>(ptr) - sizeof(size_t),
                sizeof(total_size));
