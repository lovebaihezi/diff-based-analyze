...
template <typename Arg> void codec_not_found_for_type() {
  static_assert(
      always_false_v<Arg>,
      "\n"
      "+-----------------------------------------------------------------------"
      ...
      "   - Ensure you have included the necessary headers for the specific "
      "STL type you are using "
      ...
      "\n"
      "Note: The specific type of 'Arg' can be found in the compiler error "
      "message.\n"
      "      Look for the instantiation of 'codec_not_found_for_type<Arg>' in "
      "the error output.\n"
      "      The compiler should indicate what type 'Arg' represents in this "
      "instance.\n"
      "\n"
      "For more information see "
      "https://quillcpp.readthedocs.io/en/latest/cheat_sheet.html\n");
}

QUILL_NODISCARD inline size_t safe_strnlen(char const *str,
...
template <typename Arg, typename = void> struct Codec {
  /***/
  QUILL_NODISCARD QUILL_ATTRIBUTE_HOT static size_t compute_encoded_size(
    ...
    if constexpr (std::disjunction_v<std::is_arithmetic<Arg>, std::is_enum<Arg>,
                                     std::is_same<Arg, void const *>>) {
      return sizeof(Arg);
    } else if constexpr (std::conjunction_v<
                             std::is_array<Arg>,
                             std::is_same<detail::remove_cvref_t<
                                              std::remove_extent_t<Arg>>,
                                          char>>) {
      size_t constexpr N = std::extent_v<Arg>;
      assert(((detail::safe_strnlen(arg, N) + 1u) <=
              std::numeric_limits<uint32_t>::max()) &&
             "len is outside the supported range");
      return conditional_arg_size_cache.push_back(
          static_cast<uint32_t>(detail::safe_strnlen(arg, N) + 1u));
    } else if constexpr (std::disjunction_v<std::is_same<Arg, char *>,
                                            std::is_same<Arg, char const *>>) {
      // include one extra for the zero termination
      assert(((strlen(arg) + 1u) <= std::numeric_limits<uint32_t>::max()) &&
             "len is outside the supported range");
      return conditional_arg_size_cache.push_back(
          static_cast<uint32_t>(strlen(arg) + 1u));
    } else if constexpr (std::disjunction_v<
                             detail::is_std_string<Arg>,
                             std::is_same<Arg, std::string_view>>) {
      // for std::string we also need to store the size in order to correctly
      // retrieve it the reason for this is that if we create e.g: std::string
      // msg = fmtquill::format("{} {} {} {} {}", (char)0, (char)0, (char)0,
      // (char)0, "sssssssssssssssssssssss"); then strlen(msg.data()) = 0 but
      // msg.size() = 31
  ...
  /***/
  QUILL_ATTRIBUTE_HOT static void
  encode(std::byte *&buffer,
         QUILL_MAYBE_UNUSED detail::SizeCacheVector const
    ...
    if constexpr (std::disjunction_v<std::is_arithmetic<Arg>, std::is_enum<Arg>,
      ...
      buffer += len;
                             ...
                             detail::is_std_string<Arg>,
                             std::is_same<Arg, std::string_view>>) {
      // for std::string we store the size first, in order to correctly retrieve
      // it Copy the length first and then the actual string
  ...
  // We use two separate functions, decode_arg and decode_and_store_arg, because
  // there are scenarios where we need to decode an argument without storing it
  // in args_store, such as when dealing with nested types. Storing the argument
  // requires a fmtquill formatter, so having two distinct functions allows us
  // to avoid this requirement in cases where only decoding is needed.
  ...
  static auto decode_arg(std::byte *&buffer) {
    if constexpr (std::disjunction_v<std::is_arithmetic<Arg>, std::is_enum<Arg>,
      ...
      return arg;
        ...
        std::disjunction_v<
            std::is_same<Arg, char *>, std::is_same<Arg, char const *>,
            std::conjunction<
                std::is_array<Arg>,
                std::is_same<detail::remove_cvref_t<std::remove_extent_t<Arg>>,
                             char>>>) {
      // c strings or char array
      char const *arg = reinterpret_cast<char const *>(buffer);
      buffer += strlen(arg) + 1; // for c_strings we add +1 to the length as we
                                 // also want to copy the null terminated char
      return arg;
    } else if constexpr (std::disjunction_v<
                             detail::is_std_string<Arg>,
                             std::is_same<Arg, std::string_view>>) {
      ...
      std::string_view const arg =
          std::string_view{reinterpret_cast<char const *>(buffer), len};
      buffer += len;
      return arg;
    } else {
      detail::codec_not_found_for_type<Arg>();
  ...
  static void decode_and_store_arg(std::byte *&buffer,
    ...
    if constexpr (std::disjunction_v<std::is_arithmetic<Arg>, std::is_enum<Arg>,
      ...
      // we pass fmtquill::string_view since fmt/base.h includes a formatter for
      // that type. for std::string_view we would need fmt/format.h
 ...
 * @param conditional_arg_size_cache Storage to avoid repeating calculations eg.
 cache strlen
...
compute_encoded_size_and_cache_string_lengths(
  ...
  if constexpr (!std::conjunction_v<std::disjunction<
                    std::is_arithmetic<detail::remove_cvref_t<Args>>,
                    std::is_enum<detail::remove_cvref_t<Args>>,
                    std::is_same<detail::remove_cvref_t<Args>, void const *>,
                    detail::is_std_string<detail::remove_cvref_t<Args>>,
                    std::is_same<detail::remove_cvref_t<Args>,
                                 std::string_view>>...>) {
  ...
  size_t total_sum{0};
  // Avoid using a fold expression with '+ ...' because we require a guaranteed
  // evaluation order to ensure that each argument is processed in sequence.
  // This is essential for correctly populating the conditional_arg_size_cache
  ((total_sum += Codec<detail::remove_cvref_t<Args>>::compute_encoded_size(
...
encode(std::byte *&buffer,
  ...
  (Codec<detail::remove_cvref_t<Args>>::encode(
       buffer, conditional_arg_size_cache, conditional_arg_size_cache_index,
       args),
   ...);
}

template <typename... Args>
void decode_and_store_arg(
    std::byte *&buffer, QUILL_MAYBE_UNUSED DynamicFormatArgStore *args_store) {
...
template <typename... Args>
void decode_and_store_args(std::byte *&buffer,
                           DynamicFormatArgStore &args_store) {
  args_store.clear();
...
size_t
compute_total_encoded_size(detail::SizeCacheVector &conditional_arg_size_cache,
                           TMembers const &...members) {
  size_t total_size{0};
  ((total_size += Codec<detail::remove_cvref_t<TMembers>>::compute_encoded_size(
        conditional_arg_size_cache, members)),
   ...);
  return total_size;
}
...
void encode_members(std::byte *&buffer,
                    ...
                    TMembers const &...members) {
  ((Codec<detail::remove_cvref_t<TMembers>>::encode(
       buffer, conditional_arg_size_cache, conditional_arg_size_cache_index,
       members)),
   ...);
}
...
void decode_members(std::byte *&buffer, T &, TMembers &...members) {
  // T& arg is not used but if we remove it, it will crash all users who are
  // passing the extra argument without a compile time error
