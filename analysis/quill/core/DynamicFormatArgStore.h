...
class DynamicArgList {
  ...
  template <typename T> struct TypedNode : Node {
    template <typename Arg> explicit TypedNode(Arg const &arg) : value(arg) {}
...
public:
  template <typename T, typename Arg> T const &push(Arg const &arg) {
...
private:
  ...
  template <typename T> void emplace_arg(T const &arg) {
    _data.emplace_back(
...
public:
  ...
  QUILL_NODISCARD fmtquill::basic_format_arg<fmtquill::format_context> const *
  data() const {
    return _data.data();
  }
  ...
  /**
    ...
    **Example**::
      ...
      fmtquill::dynamic_format_arg_store<fmtquill::format_fmtquill::format_context>
    store; store.push_back(42); store.push_back("abc"); store.push_back(1.5f);
      std::string result = fmtquill::vformat("{} and {} and {}", store);
  */
  template <typename T> void push_back(T const &arg) {
    constexpr auto mapped_type =
        fmtquill::detail::mapped_type_constant<T,
                                               fmtquill::format_context>::value;
    using stored_type =
        std::conditional_t<std::is_convertible_v<T, std::string>, std::string,
                           T>;

    if constexpr (!(std::is_same_v<std::remove_cv_t<std::remove_reference_t<T>>,
                                   std::string_view> ||
                    std::is_same_v<std::remove_cv_t<std::remove_reference_t<T>>,
                                   fmtquill::string_view> ||
