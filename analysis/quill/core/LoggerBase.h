...
public:
   ...
   * @return The ClockSourceType enum value representing the current clock
   * source.
   */
   ...
   */
  QUILL_NODISCARD PatternFormatterOptions const &
  get_pattern_formatter_options() const noexcept {
    return pattern_formatter_options;
  }

  /**
   * Returns a const reference to the sinks vector.
   * @warning The returned sinks should be treated as read-only. Do not modify
   * the Sink objects as they are used by the backend thread
   * @return A constant reference to the std::vector<std::shared_ptr<Sink>>
   * object
   */
  QUILL_NODISCARD std::vector<std::shared_ptr<Sink>> const &
  get_sinks() const noexcept {
    return sinks;
  }

  /**
   * This function sets the logger's validity flag to false, indicating that the
   * logger is no longer valid.
   */
  ...
  should_log_statement() const noexcept {
   ...
   */
  QUILL_NODISCARD QUILL_ATTRIBUTE_HOT bool
  should_log_statement(LogLevel log_statement_level) const noexcept {
    return log_statement_level >= get_log_level();
  }
...
protected:
  ...
  static inline QUILL_THREAD_LOCAL ThreadContext *thread_context =
      nullptr; /* Set and accessed by the frontend */
  std::shared_ptr<PatternFormatter>
      pattern_formatter; /* The backend thread will set this once, we never
                            access it on the frontend */
  std::shared_ptr<BacktraceStorage>
      backtrace_storage; /* The backend thread will construct this, we never
                            access it on the frontend */
  std::vector<std::shared_ptr<Sink>>
      sinks;               /* Set by the frontend and accessed by the backend */
  std::string logger_name; /* Set by the frontend, accessed by the frontend AND
                              backend */
  UserClockSource *user_clock{
      nullptr}; /* A non owned pointer to a custom timestamp clock, valid only
                   when provided. used by frontend only */
  PatternFormatterOptions
      pattern_formatter_options; /* Set by the frontend and accessed by the
                                    backend to initialise PatternFormatter */
  ClockSourceType clock_source;  /* Set by the frontend and accessed by the
                                    frontend AND backend */
  std::atomic<LogLevel> log_level{LogLevel::Info}; /* used by frontend only */
  std::atomic<LogLevel> backtrace_flush_level{
      LogLevel::None}; /** Updated by the frontend at any time, accessed by the
                          backend */
  std::atomic<bool> valid{
      true}; /* Updated by the frontend at any time, accessed by the backend */
};
} // namespace detail
