...
ReturnT callRunTimeDynamicLinkedFunction(std::string const &dll_name,
  ...
  // Windows Server 2016, Windows 10 LTSB 2016 and Windows 10 version 1607: e.g.
  ...
  auto const callable = reinterpret_cast<Signature>(
      GetProcAddress(hinstLibrary, function_name.c_str()));

  if (QUILL_UNLIKELY(callable == nullptr)) {
    FreeLibrary(hinstLibrary);
    QUILL_THROW(QuillError{
        std::string{"Failed to call " + function_name + " " + dll_name}});
  }
...
QUILL_NODISCARD inline std::string get_thread_name() {
  ...
  HRESULT hr =
      callRunTimeDynamicLinkedFunction<HRESULT, GetThreadDescriptionSignature>(
  ...
  if (res != 0) {
    QUILL_THROW(
        QuillError{"Failed to get thread name. error: " + std::to_string(res)});
  }
...
QUILL_NODISCARD inline uint32_t get_thread_id() noexcept {
  ...
  return reinterpret_cast<uintptr_t>(
      pthread_self()); // (Ab)use pthread_self as a last resort option
#endif
}
} // namespace detail
