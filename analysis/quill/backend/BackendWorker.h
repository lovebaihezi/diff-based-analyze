...
public:
   ...
   */
  ~BackendWorker() {
    // This destructor will run during static destruction as the thread is part
    // of the singleton
    stop();
  ...
  /**
   * Access the rdtsc class from any thread to convert an rdtsc value to wall
   * time
   */
  QUILL_NODISCARD uint64_t time_since_epoch(uint64_t rdtsc_value) const {
    if (QUILL_UNLIKELY(_options.sleep_duration >
                       _options.rdtsc_resync_interval)) {
      QUILL_THROW(QuillError{"Invalid config, When TSC clock is used "
                             "backend_thread_sleep_duration should "
                             "not be higher than rdtsc_resync_interval"});
    }

    RdtscClock const *rdtsc_clock =
        _rdtsc_clock.load(std::memory_order_acquire);
  ...
  QUILL_ATTRIBUTE_COLD void run(BackendOptions const &options) {
    std::thread worker([this, &options]() {
      _init(options);
  ...
  QUILL_ATTRIBUTE_COLD void stop() noexcept {
    ...
    // Wait the backend thread to join, if backend thread was never started it
    // won't be joinable
...
private:
  ...
  QUILL_ATTRIBUTE_HOT void _poll() {
    ...
    // Read all frontend queues and cache the log statements and the metadata as
    // TransitEvents
    size_t const cached_transit_events_count =
    ...
    if (cached_transit_events_count != 0) {
      ...
      if (cached_transit_events_count < _options.transit_events_soft_limit) {
        // process a single transit event, then give priority to reading the
        // frontend queues again
        _process_lowest_timestamp_transit_event();
      } else {
        // we want to process a batch of events.
        while (
            !has_pending_events_for_caching_when_transit_event_buffer_empty() &&
            _process_lowest_timestamp_transit_event()) {
          // We need to be cautious because there are log messages in the
          // lock-free queues that have not yet been cached in the transit event
          // buffer. Logging only the cached messages can result in out-of-order
          // log entries, as messages with larger timestamps in the queue might
          // be missed.
        }
      }
    } else {
      ...
      bool const queues_and_events_empty =
      ...
      if (queues_and_events_empty) {
        ...
        // There is nothing left to do, and we can let this thread sleep for a
        // while buffer events are 0 here and also all the producer queues are
        // empty
        if (_options.sleep_duration.count() != 0) {
          ...
          _wake_up_cv.wait_for(lock, _options.sleep_duration,
  ...
  QUILL_ATTRIBUTE_COLD void _init(BackendOptions const &options) {
    ...
    if (_options.transit_events_hard_limit == 0) {
      // transit_events_hard_limit of 0 makes no sense as we can't process
      // anything
    ...
    if (_options.transit_events_soft_limit >
        _options.transit_events_hard_limit) {
      QUILL_THROW(QuillError{
          fmtquill::format("transit_events_soft_limit ({}) cannot be greater "
                           "than transit_events_hard_limit "
                           "({}). Please ensure that the soft limit is less "
                           "than or equal to the hard limit.",
                           _options.transit_events_soft_limit,
                           _options.transit_events_hard_limit)});
    } else if (!is_power_of_two(_options.transit_events_hard_limit)) {
      QUILL_THROW(QuillError{fmtquill::format(
          "transit_events_hard_limit ({}) must be a power of two",
          _options.transit_events_hard_limit)});
    } else if (!is_power_of_two(_options.transit_events_soft_limit)) {
      QUILL_THROW(QuillError{fmtquill::format(
          "transit_events_soft_limit ({}) must be a power of two",
          _options.transit_events_soft_limit)});
    }
  }
  ...
  QUILL_ATTRIBUTE_COLD void _exit() {
    while (true) {
      ...
      uint64_t const cached_transit_events_count =
          _populate_transit_events_from_frontend_queues();
      if (cached_transit_events_count > 0) {
        while (
            !has_pending_events_for_caching_when_transit_event_buffer_empty() &&
  ...
  QUILL_ATTRIBUTE_HOT size_t _populate_transit_events_from_frontend_queues() {
    uint64_t const ts_now =
        _options.log_timestamp_ordering_grace_period.count()
    ...
    for (ThreadContext *thread_context : _active_thread_contexts_cache) {
      assert(thread_context->has_unbounded_queue_type() ||
      ...
      if (thread_context->has_unbounded_queue_type()) {
        cached_transit_events_count += _read_and_decode_frontend_queue(
            thread_context->get_spsc_queue_union().unbounded_spsc_queue,
            thread_context, ts_now);
      } else if (thread_context->has_bounded_queue_type()) {
        cached_transit_events_count += _read_and_decode_frontend_queue(
            thread_context->get_spsc_queue_union().bounded_spsc_queue,
            thread_context, ts_now);
      }
    }
  ...
  QUILL_ATTRIBUTE_HOT size_t _read_and_decode_frontend_queue(
      TFrontendQueue &frontend_queue, ThreadContext *thread_context,
      uint64_t ts_now) {
    // Note: The producer commits only complete messages to the queue.
    // Therefore, if even a single byte is present in the queue, it signifies a
    // full message.
    ...
    do {
      ...
      if constexpr (std::is_same_v<TFrontendQueue, UnboundedSPSCQueue>) {
        read_pos =
            _read_unbounded_frontend_queue(frontend_queue, thread_context);
      } else {
      ...
      if (!_populate_transit_event_from_frontend_queue(read_pos, thread_context,
                                                       ts_now)) {
      ...
      // Reads a maximum of one full frontend queue or the transit events' hard
      // limit to prevent getting stuck on the same producer.
    } while ((total_bytes_read < queue_capacity) &&
             (thread_context->_transit_event_buffer->size() <
              _options.transit_events_hard_limit));

    if (total_bytes_read != 0) {
      // If we read something from the queue, we commit all the reads together
      // at the end. This strategy enhances cache coherence performance by
      // updating the shared atomic flag only once.
  ...
  /***/
  QUILL_ATTRIBUTE_HOT bool _populate_transit_event_from_frontend_queue(
      std::byte *&read_pos, ThreadContext *thread_context, uint64_t ts_now) {
    assert(thread_context->_transit_event_buffer);

    // Allocate a new TransitEvent or use an existing one to store the message
    // from the queue
    TransitEvent *transit_event = thread_context->_transit_event_buffer->back();

    std::memcpy(&transit_event->timestamp, read_pos,
                sizeof(transit_event->timestamp));
    read_pos += sizeof(transit_event->timestamp);

    std::memcpy(&transit_event->macro_metadata, read_pos,
                sizeof(transit_event->macro_metadata));
    read_pos += sizeof(transit_event->macro_metadata);

    std::memcpy(&transit_event->logger_base, read_pos,
    ...
    if (transit_event->logger_base->clock_source == ClockSourceType::Tsc) {
      ...
      // This conversion ensures that every transit inserted in the buffer below
      // has a timestamp in nanoseconds since epoch, allowing compatibility with
      // Logger objects using different clocks.
      if (QUILL_UNLIKELY(!_rdtsc_clock.load(std::memory_order_relaxed))) {
        // Lazy initialization of rdtsc clock on the backend thread only if the
        // user decides to use it. The clock requires a few seconds to init as
        // it is taking samples first.
        _rdtsc_clock.store(new RdtscClock{_options.rdtsc_resync_interval},
      ...
      transit_event->timestamp =
          _rdtsc_clock.load(std::memory_order_relaxed)
              ->time_since_epoch(transit_event->timestamp);
    }

    // Check if strict log timestamp order is enabled and the clock source is
    // not User
    if ((transit_event->logger_base->clock_source != ClockSourceType::User) &&
        (ts_now != std::numeric_limits<uint64_t>::max())) {
      // We only check against `ts_now` for real timestamps, not custom
      // timestamps by the user, and when the grace period is enabled
      ...
      if (QUILL_UNLIKELY(transit_event->timestamp > ts_now)) {
        // If the message timestamp is ahead of the current time, temporarily
        // halt processing. This guarantees the integrity of message order and
        // avoids missed messages. We halt processing here to avoid introducing
        // out-of-sequence messages. This scenario prevents potential race
        // conditions where timestamps from the last queue could overwrite those
        // from the first queue before they are included. We return at this
        // point without adding the current event to the buffer.
        return false;
      }
    }
    ...
    // we need to check and do not try to format the flush events as that
    // wouldn't be valid
    if (transit_event->macro_metadata->event() != MacroMetadata::Event::Flush) {
      ...
      if (!transit_event->macro_metadata->has_named_args()) {
        _populate_formatted_log_message(
            transit_event, transit_event->macro_metadata->message_format());
      } else {
        // using the message_format as key for lookups
        _named_args_format_template.assign(
            transit_event->macro_metadata->message_format());

        if (auto const search =
                _named_args_templates.find(_named_args_format_template);
            search != std::cend(_named_args_templates)) {
          // process named args message when we already have parsed the format
          // message once, and we have the names of each arg cached
          ...
          // process named args log when the message format is processed for the
          // first time parse name of each arg and stored them to our lookup map
          auto const [res_it, inserted] = _named_args_templates.try_emplace(
              _named_args_format_template,
              _process_named_args_format_message(
                  transit_event->macro_metadata->message_format()));
      ...
      // transit_event, but we need to set the transit event's flush_flag
      // pointer instead
      uintptr_t flush_flag_tmp;
      std::memcpy(&flush_flag_tmp, read_pos, sizeof(uintptr_t));
      transit_event->flush_flag =
          reinterpret_cast<std::atomic<bool> *>(flush_flag_tmp);
      read_pos += sizeof(uintptr_t);
    }

    if (transit_event->macro_metadata->log_level() == LogLevel::Dynamic) {
      // if this is a dynamic log level we need to read the log level from the
      // buffer
      std::memcpy(&transit_event->dynamic_log_level, read_pos,
                  sizeof(transit_event->dynamic_log_level));
  ...
  /**
   * Checks if there are pending events for caching based on the state of
   * transit event buffers and queues.
   * @return True if there are pending events for caching when the
   * _transit_event_buffer is empty, false otherwise.
   */
  QUILL_ATTRIBUTE_HOT bool
  has_pending_events_for_caching_when_transit_event_buffer_empty() noexcept {
    _update_active_thread_contexts_cache();

    for (ThreadContext *thread_context : _active_thread_contexts_cache) {
      assert(thread_context->_transit_event_buffer &&
             "transit_event_buffer should always be valid here as we always "
      ...
      if (thread_context->_transit_event_buffer->empty()) {
        ...
        if (thread_context->has_unbounded_queue_type() &&
            !thread_context->get_spsc_queue_union()
                 .unbounded_spsc_queue.empty()) {
          return true;
        }

        if (thread_context->has_bounded_queue_type() &&
            !thread_context->get_spsc_queue_union()
                 .bounded_spsc_queue.empty()) {
          return true;
        }
      }
    }
  ...
  QUILL_ATTRIBUTE_HOT bool _process_lowest_timestamp_transit_event() {
    ...
    TransitEvent *transit_event =
        thread_context->_transit_event_buffer->front();
    assert(transit_event &&
           "transit_buffer is set only when transit_event is valid");

    std::atomic<bool> *flush_flag{nullptr};

    QUILL_TRY {
      _process_transit_event(*thread_context, *transit_event, flush_flag);
    }
#if !defined(QUILL_NO_EXCEPTIONS)
    ...
    if (flush_flag) {
      // Process the second part of the flush event after it's been removed from
      // the buffer, ensuring that we are no longer interacting with the
      // thread_context or transit_event.

      // This is particularly important for handling cases when Quill is used as
      // a DLL on Windows. If `FreeLibrary` is called, the backend thread may
      // attempt to access an invalidated `ThreadContext`, which can lead to a
      // crash due to invalid memory access.
      //
      // To prevent this, whenever we receive a Flush event, we clean up any
      // invalidated thread contexts before notifying the caller. This ensures
      // that when `flush_log()` is invoked in `DllMain` during
      // `DLL_PROCESS_DETACH`, the `ThreadContext` is properly cleaned up before
      // the DLL exits.
      _cleanup_invalidated_thread_contexts();
  ...
  _process_transit_event(ThreadContext const &thread_context,
                         TransitEvent &transit_event,
                         std::atomic<bool> *&flush_flag) {
    // If backend_process(...) throws we want to skip this event and move to the
    // next, so we catch the error here instead of catching it in the parent
    // try/catch block of main_loop
    if (transit_event.macro_metadata->event() == MacroMetadata::Event::Log) {
      if (transit_event.log_level() != LogLevel::Backtrace) {
        ...
        // We also need to check the severity of the log message here against
        // the backtrace Check if we should also flush the backtrace messages:
        // After we forwarded the message we will check the severity of this
        // message for this logger If the severity of the message is higher than
        // the backtrace flush severity we will also flush the backtrace of the
        // logger
        if (QUILL_UNLIKELY(
                transit_event.log_level() >=
                transit_event.logger_base->backtrace_flush_level.load(
                    std::memory_order_relaxed))) {
          if (transit_event.logger_base->backtrace_storage) {
            transit_event.logger_base->backtrace_storage->process(
                [this](TransitEvent const &te, std::string_view thread_id,
        ...
        if (transit_event.logger_base->backtrace_storage) {
          ...
          transit_event.logger_base->backtrace_storage->store(
              std::move(transit_event), thread_context.thread_id(),
              thread_context.thread_name());
        } else {
          QUILL_THROW(QuillError{"logger->init_backtrace(...) needs to be "
                                 "called first before using "
                                 "LOG_BACKTRACE(...)."});
        }
      }
    } else if (transit_event.macro_metadata->event() ==
               MacroMetadata::Event::InitBacktrace) {
      // we can just convert the capacity back to int here and use it
      if (!transit_event.logger_base->backtrace_storage) {
        // Lazy BacktraceStorage creation
        transit_event.logger_base->backtrace_storage =
            std::make_shared<BacktraceStorage>();
      }

      transit_event.logger_base->backtrace_storage->set_capacity(
          static_cast<uint32_t>(
              std::stoul(std::string{transit_event.formatted_msg->begin(),
                                     transit_event.formatted_msg->end()})));
    } else if (transit_event.macro_metadata->event() ==
               MacroMetadata::Event::FlushBacktrace) {
      if (transit_event.logger_base->backtrace_storage) {
      ...
      // This is a flush event, so we capture the flush flag to notify the
      // caller after processing.
      flush_flag = transit_event.flush_flag;

      // Reset the flush flag as TransitEvents are re-used, preventing incorrect
      // flag reuse.
   ...
   */
  QUILL_ATTRIBUTE_HOT void
  _dispatch_transit_event_to_sinks(TransitEvent const &transit_event,
                                   std::string_view const &thread_id,
                                   std::string_view const &thread_name) {
    // First check to see if we should init the pattern formatter on a new
    // Logger Look up to see if we have the formatter and if not create it
    if (QUILL_UNLIKELY(!transit_event.logger_base->pattern_formatter)) {
      // Search for an existing pattern_formatter in each logger
      _logger_manager.for_each_logger([&transit_event](LoggerBase *logger) {
        if (logger->pattern_formatter &&
            (logger->pattern_formatter->get_options() ==
             transit_event.logger_base->pattern_formatter_options)) {
          // hold a copy of the shared_ptr of the same formatter
          transit_event.logger_base->pattern_formatter =
              logger->pattern_formatter;
          return true;
        }
      ...
      if (!transit_event.logger_base->pattern_formatter) {
        // Didn't find an existing formatter  need to create a new pattern
        // formatter
        transit_event.logger_base->pattern_formatter =
            std::make_shared<PatternFormatter>(
                transit_event.logger_base->pattern_formatter_options);
      }
    }
    ...
    std::string_view const log_level_description = detail::log_level_to_string(
        transit_event.log_level(), _options.log_level_descriptions.data(),
        _options.log_level_descriptions.size());

    std::string_view const log_level_short_code = detail::log_level_to_string(
        transit_event.log_level(), _options.log_level_short_codes.data(),
        _options.log_level_short_codes.size());

    if (transit_event.logger_base->pattern_formatter->get_options()
      ...
      _process_multi_line_message(transit_event, thread_id, thread_name,
      ...
      size_t const log_message_size =
          transit_event.formatted_msg
                      ->data()[transit_event.formatted_msg->size() - 1] == '\n'
      ...
      _write_log_statement(transit_event, thread_id, thread_name,
                           log_level_description, log_level_short_code,
                           std::string_view{transit_event.formatted_msg->data(),
                                            log_message_size});
    }
  }
   ...
   */
  QUILL_ATTRIBUTE_HOT void _process_multi_line_message(
      TransitEvent const &transit_event, std::string_view const &thread_id,
      std::string_view const &thread_name,
      std::string_view const &log_level_description,
      std::string_view const &log_level_short_code) const {
    auto const msg = std::string_view{transit_event.formatted_msg->data(),
                                      transit_event.formatted_msg->size()};
    ...
    while (start < msg.size()) {
      ...
      if (end == std::string_view::npos) {
        ...
        _write_log_statement(
            transit_event, thread_id, thread_name, log_level_description,
   ...
   */
  QUILL_ATTRIBUTE_HOT void
  _write_log_statement(TransitEvent const &transit_event,
                       std::string_view const &thread_id,
                       std::string_view const &thread_name,
                       std::string_view const &log_level_description,
                       std::string_view const &log_level_short_code,
                       std::string_view const &log_message) const {
    std::string_view const log_statement =
        transit_event.logger_base->pattern_formatter->format(
            transit_event.timestamp, thread_id, thread_name, _process_id,
            transit_event.logger_base->logger_name, log_level_description,
            log_level_short_code, *transit_event.macro_metadata,
            transit_event.named_args.get(), log_message);

    for (auto &sink : transit_event.logger_base->sinks) {
      if (sink->apply_all_filters(
              transit_event.macro_metadata, transit_event.timestamp, thread_id,
              thread_name, transit_event.logger_base->logger_name,
              transit_event.log_level(), log_message, log_statement)) {
        sink->write_log(transit_event.macro_metadata, transit_event.timestamp,
                        thread_id, thread_name, _process_id,
                        transit_event.logger_base->logger_name,
                        transit_event.log_level(), log_level_description,
                        log_level_short_code, transit_event.named_args.get(),
  ...
  QUILL_ATTRIBUTE_HOT void _check_failure_counter(
      std::function<void(std::string const &)> const &error_notifier) noexcept {
    // UnboundedNoMaxLimit does not block or drop messages
    for (ThreadContext *thread_context : _active_thread_contexts_cache) {
      if (thread_context->has_bounded_queue_type()) {
        size_t const failed_messages_cnt =
            thread_context->get_and_reset_failure_counter();
        ...
        if (QUILL_UNLIKELY(failed_messages_cnt > 0)) {
          ...
          if (thread_context->has_dropping_queue()) {
            ...
            error_notifier(fmtquill::format("{} Quill INFO: Experienced {} "
                                            "blocking occurrences on thread {}",
                                            timestamp, failed_messages_cnt,
                                            thread_context->thread_id()));
          }
        }
      }
   ...
   * @return first: fmt string without the named arguments, second: a vector
   * extracted keys
   */
  QUILL_ATTRIBUTE_HOT static std::pair<
      std::string, std::vector<std::pair<std::string, std::string>>>
  _process_named_args_format_message(std::string_view fmt_template) noexcept {
    // It would be nice to do this at compile time and store it in macro
    // metadata, but without constexpr vector and string in c++17 it is not
    // possible
    std::string fmt_str;
    ...
    while (open_bracket_pos != std::string::npos) {
      ...
      if (size_t const open_bracket_2_pos =
              fmt_template.find_first_of('{', open_bracket_pos + 1);
          open_bracket_2_pos != std::string::npos) {
        // found another open bracket
        if ((open_bracket_2_pos - 1) == open_bracket_pos) {
          open_bracket_pos =
              fmt_template.find_first_of('{', open_bracket_2_pos + 1);
          continue;
        }
      }
      ...
      while (close_bracket_pos != std::string::npos) {
        ...
        if (size_t const close_bracket_2_pos =
                fmt_template.find_first_of('}', close_bracket_pos + 1);
            close_bracket_2_pos != std::string::npos) {
          // found another open bracket
          if ((close_bracket_2_pos - 1) == close_bracket_pos) {
            close_bracket_pos =
                fmt_template.find_first_of('}', close_bracket_2_pos + 1);
            continue;
          }
        }
        ...
        // }
        std::string_view const text_inside_placeholders = fmt_template.substr(
            open_bracket_pos + 1, close_bracket_pos - (open_bracket_pos + 1));
        std::string_view arg_syntax;
        std::string_view arg_name;

        // look in text_inside_placeholders for special syntax formating
        // following the named arg e.g. arg:.2f
        if (size_t const syntax_separator = text_inside_placeholders.find(':');
          ...
          arg_syntax = text_inside_placeholders.substr(
              syntax_separator,
              text_inside_placeholders.size() - syntax_separator);
        ...
        fmt_str += fmtquill::format(
            "{}{{{}}}",
            fmt_template.substr(cur_pos, open_bracket_pos - cur_pos),
            arg_syntax);
    ...
    fmt_str += std::string{
        fmt_template.substr(cur_pos, fmt_template.length() - cur_pos)};
  ...
  _read_unbounded_frontend_queue(UnboundedSPSCQueue &frontend_queue,
                                 ThreadContext *thread_context) const {
    auto const read_result = frontend_queue.prepare_read();

    if (read_result.allocation) {
      // When allocation_info has a value it means that the queue has
      // re-allocated
      if (_options.error_notifier) {
        ...
        // we switched to a new here, and we also notify the user of the
        // allocation via the error_notifier
        _options.error_notifier(
            fmtquill::format("{} Quill INFO: Allocated a new SPSC queue with a "
                             "capacity of {} KiB "
                             "(previously {} KiB) from thread {}",
                             ts, (read_result.new_capacity / 1024),
                             (read_result.previous_capacity / 1024),
                             thread_context->thread_id()));
      }
    }
  ...
  QUILL_NODISCARD QUILL_ATTRIBUTE_HOT bool
  _check_frontend_queues_and_cached_transit_events_empty() {
    _update_active_thread_contexts_cache();
  ...
  QUILL_ATTRIBUTE_HOT void _resync_rdtsc_clock() {
    if (_rdtsc_clock.load(std::memory_order_relaxed)) {
      // resync in rdtsc if we are not logging so that time_since_epoch() still
      // works
  ...
  /***/
  QUILL_ATTRIBUTE_HOT void
  _flush_and_run_active_sinks(bool run_periodic_tasks) {
    // Populate the active sinks cache with unique sinks, consider only the
    // valid loggers
    _logger_manager.for_each_logger([this](LoggerBase *logger) {
      if (logger->is_valid_logger()) {
        for (std::shared_ptr<Sink> const &sink : logger->sinks) {
          Sink *logger_sink_ptr = sink.get();
          auto search_it = std::find_if(_active_sinks_cache.begin(),
                                        _active_sinks_cache.end(),
                                        [logger_sink_ptr](Sink *elem) {
                                          // no one else can remove the shared
                                          // pointer as this is only running on
                                          // backend thread
    ...
    for (auto const &sink : _active_sinks_cache) {
      QUILL_TRY {
        // If an exception is thrown, catch it here to prevent it from
        // propagating to the outer function. This prevents potential infinite
        // loops caused by failing flush operations.
        sink->flush_sink();
      }
#if !defined(QUILL_NO_EXCEPTIONS)
      QUILL_CATCH(std::exception const &e) {
  ...
  QUILL_ATTRIBUTE_HOT void _update_active_thread_contexts_cache() {
    // Check if _thread_contexts has changed. This can happen only when a new
    // thread context is added by any Logger
      ...
      _thread_context_manager.for_each_thread_context(
          [this](ThreadContext *thread_context) {
            if (!thread_context->_transit_event_buffer) {
              // Lazy initialise the _transit_event_buffer for this
              // thread_context
              thread_context->_transit_event_buffer =
                  std::make_shared<TransitEventBuffer>(
                      _options.transit_event_buffer_initial_capacity);
            }

            // We do not skip invalidated && empty queue thread contexts as this
            // is very rare, so instead we just add them and expect them to be
            // cleaned in the next iteration
  ...
  /**
   * Looks into the _thread_context_cache and removes all thread contexts that
   * are 1) invalidated and 2) have an empty frontend queue and no cached
   * transit events to process
   *
  ...
  QUILL_ATTRIBUTE_HOT void _cleanup_invalidated_thread_contexts() {
    ...
    auto find_invalid_and_empty_thread_context_callback =
        [](ThreadContext *thread_context) {
          // If the thread context is invalid it means the thread that created
          // it has now died. We also want to empty the queue from all
          // LogRecords before removing the thread context
          if (!thread_context->is_valid()) {
            assert(thread_context->has_unbounded_queue_type() ||
                   thread_context->has_bounded_queue_type());

            assert(thread_context->_transit_event_buffer &&
                   "transit_event_buffer should always be valid here as we "
                   "always populate it with the "
                   "_active_thread_contexts_cache");

            // detect empty queue
            if (thread_context->has_unbounded_queue_type()) {
              return thread_context->get_spsc_queue_union()
                         .unbounded_spsc_queue.empty() &&
    ...
    // First we iterate our existing cache and we look for any invalidated
    // contexts
    auto found_invalid_and_empty_thread_context =
        std::find_if(_active_thread_contexts_cache.begin(),
                     _active_thread_contexts_cache.end(),
                     find_invalid_and_empty_thread_context_callback);

    while (QUILL_UNLIKELY(found_invalid_and_empty_thread_context !=
                          std::end(_active_thread_contexts_cache))) {
      // if we found anything then remove it - Here if we have more than one to
      // remove we will try to acquire the lock multiple times, but it should be
      // fine as it is unlikely to have that many to remove
      _thread_context_manager.remove_shared_invalidated_thread_context(
          *found_invalid_and_empty_thread_context);

      // We also need to remove it from _thread_context_cache, that is used only
      // by the backend
      _active_thread_contexts_cache.erase(
          found_invalid_and_empty_thread_context);
  ...
  QUILL_ATTRIBUTE_HOT void _cleanup_invalidated_loggers() {
    // since there are no messages we can check for invalidated loggers and
    // clean them up
    std::vector<std::string> const removed_loggers =
        _logger_manager.cleanup_invalidated_loggers([this]() {
          // check the queues are empty each time before removing a logger to
          // avoid potential race condition of the logger* still being in the
          // queue
    ...
    if (!removed_loggers.empty()) {
      ...
      // cleanup_unused_sinks is expensive and should be only called when it is
      // needed
  ...
  /**
   * This function takes an `format_args_store` containing multiple arguments
   * and formats them into a single string using a generated format string. Due
   * to limitations in the ability to iterate and format each argument
   * individually in libfmt, this approach is used. After formatting, the string
   * is split to isolate each formatted value.
   */
  static void _format_and_split_arguments(
      std::vector<std::pair<std::string, std::string>> const &orig_arg_names,
    ...
    for (size_t i = 0; i < named_args.size(); ++i) {
      // We need an additional check here because named_args can have a size
      // greater than orig_arg_names This is because we are adding the arguments
      // without a name with a placeholder name
      if ((i < orig_arg_names.size()) && !orig_arg_names[i].second.empty()) {
        // orig_arg_names[i].second is special format syntax for the named
        // argument if provided, eg name:.2f
    ...
    while ((end = formatted_values_str.find(delimiter, start)) !=
           std::string::npos) {
      if (idx < named_args.size()) {
        named_args[idx++].second =
            formatted_values_str.substr(start, end - start);
      }
    ...
    // We call sanitize_non_printable_chars for each value, because
    // formatted_values_str already contains non-printable characters for the
    // argument separation
    if (options.check_printable_char &&
        format_args_store.has_string_related_type()) {
      // if non-printable chars check is configured or if any of the provided
  ...
  void _populate_formatted_named_args(
    ...
    for (size_t i = arg_names.size();
         i < static_cast<size_t>(_format_args_store.size()); ++i) {
      // we do not have a named_arg for the argument value here so we just
      // append its index as a placeholder
      transit_event->named_args->push_back(std::pair<std::string, std::string>(
          fmtquill::format("_{}", i), std::string{}));
    }

    // Then populate all the values of each arg
    QUILL_TRY {
      _format_and_split_arguments(arg_names, *transit_event->named_args,
                                  _format_args_store, _options);
    }
#if !defined(QUILL_NO_EXCEPTIONS)
    QUILL_CATCH(std::exception const &) {
      // This catch block simply catches the exception.
      // Since the error has already been handled in
      // _populate_formatted_log_message, there is no additional action required
      // here.
    }
#endif
  }
  ...
  QUILL_ATTRIBUTE_HOT void
  _populate_formatted_log_message(TransitEvent *transit_event,
    ...
    QUILL_TRY {
      ...
      if (_options.check_printable_char &&
          _format_args_store.has_string_related_type()) {
        // if non-printable chars check is configured or if any of the provided
        // arguments are strings
  ...
  static void sanitize_non_printable_chars(TFormattedMsg &formatted_msg,
                                           BackendOptions const &options) {
    ...
    if (contains_non_printable_char) {
      // in this rare event we will replace the non-printable chars with their
      // hex values
      std::string const formatted_msg_copy = {formatted_msg.data(),
                                              formatted_msg.size()};
      formatted_msg.clear();
...
private:
  ...
  ThreadContextManager &_thread_context_manager =
  ...
  DynamicFormatArgStore _format_args_store; /** Format args tmp storage as
  ...
  std::unordered_map<
      ...
      _named_args_templates; /** Avoid re-formating the same named args log
                                template each time */
  ...
  std::atomic<uint32_t> _worker_thread_id{
      0}; /** cached backend worker thread id */
  std::atomic<bool> _is_worker_running{
      false}; /** The spawned backend thread status */
