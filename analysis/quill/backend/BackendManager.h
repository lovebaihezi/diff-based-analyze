...
namespace detail {
/**
 * Provides access to common collection class that are used by both the frontend
 * and the backend components of the logging system There should only be only
 * active active instance of this class which is achieved by the
...
private:
  ...
  start_backend_thread(BackendOptions const &options) {
  ...
  convert_rdtsc_to_epoch_time(uint64_t rdtsc_value) const noexcept {
