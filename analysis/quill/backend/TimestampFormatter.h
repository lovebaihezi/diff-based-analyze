 ...
 * additional specifiers : 1) %Qms - Milliseconds 2) %Qus - Microseconds 3) %Qns
 * - Nanoseconds
...
private:
  enum AdditionalSpecifier : uint8_t { None, Qms, Qus, Qns };

public:
  /***/
  explicit TimestampFormatter(std::string time_format,
                              Timezone timestamp_timezone = Timezone::LocalTime)
      : _time_format(std::move(time_format)),
        _timestamp_timezone(timestamp_timezone) {
    assert((_timestamp_timezone == Timezone::LocalTime ||
            _timestamp_timezone == Timezone::GmtTime) &&
    ...
    if (size_t const search_qms =
            _time_format.find(specifier_name[AdditionalSpecifier::Qms]);
        search_qms != std::string::npos) {
      _additional_format_specifier = AdditionalSpecifier::Qms;
      specifier_begin = search_qms;
    }

    if (size_t const search_qus =
            _time_format.find(specifier_name[AdditionalSpecifier::Qus]);
    ...
    if (size_t const search_qns =
            _time_format.find(specifier_name[AdditionalSpecifier::Qns]);
    ...
    if (specifier_begin == std::string::npos) {
      // If no additional specifier was found then we can simply store the whole
      // format string
      assert(_additional_format_specifier == AdditionalSpecifier::None);
      _strftime_part_1.init(_time_format, _timestamp_timezone);
    } else {
      // We now the index where the specifier begins so copy everything until
      ...
      std::string const format_part_2 = _time_format.substr(
          specifier_end, _time_format.length() - specifier_end);
  ...
  format_timestamp(std::chrono::nanoseconds time_since_epoch) {
    ...
    auto const extracted_ns =
        static_cast<uint32_t>(timestamp_ns - (timestamp_secs * 1'000'000'000));
...
private:
  ...
  void _write_fractional_seconds(uint32_t extracted_fractional_seconds) {
    ...
    fmtquill::format_int const extracted_ms_string{
        extracted_fractional_seconds};

    // _formatted_date.size() - extracted_ms_string.size() is where we want to
    // begin placing the fractional seconds
    memcpy(
        &_formatted_date[_formatted_date.size() - extracted_ms_string.size()],
        extracted_ms_string.data(), extracted_ms_string.size());
  }

private:
  /** Contains the additional specifier name, at the same index as the enum **/
  static constexpr std::array<char const *, 4> specifier_name{"", "%Qms",
                                                              "%Qus", "%Qns"};
