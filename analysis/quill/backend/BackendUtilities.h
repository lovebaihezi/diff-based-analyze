...
QUILL_ATTRIBUTE_COLD inline void set_cpu_affinity(uint16_t cpu_id) {
  ...
  if (ret == 0) {
    ...
    QUILL_THROW(QuillError{std::string{"Failed to set cpu affinity - errno: " +
                                       std::to_string(last_error.value()) +
                                       " error: " + last_error.message()}});
  }
#elif defined(__APPLE__)
  // I don't think that's possible to link a thread with a specific core with
  // Mac OS X This may be used to express affinity relationships  between
  // threads in the task. Threads with the same affinity tag will be scheduled
  ...
  if (QUILL_UNLIKELY(err == -1)) {
    QUILL_THROW(QuillError{std::string{
        "Failed to set cpu affinity - errno: " + std::to_string(errno) +
...
QUILL_ATTRIBUTE_COLD inline void set_thread_name(char const *name) {
  ...
  HRESULT hr =
      callRunTimeDynamicLinkedFunction<HRESULT, SetThreadDescriptionSignature>(
          "KernelBase.dll", "SetThreadDescription", GetCurrentThread(),
          name_ws.data());
  ...
  if (res != 0) {
    QUILL_THROW(QuillError{std::string{
        "Failed to set thread name - errno: " + std::to_string(errno) +
        " error: " + strerror(errno)}});
  }
#else
  // linux
