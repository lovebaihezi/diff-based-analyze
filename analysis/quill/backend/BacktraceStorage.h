 ...
 * We push to the queue a BacktraceCommand event to communicate this from the
 * frontend caller threads
 */
...
public:
  ...
  /***/
  void store(TransitEvent transit_event, std::string_view const &thread_id,
             std::string_view const &thread_name) {
    if (_stored_events.size() < _capacity) {
      // We are still growing the vector to max capacity
      _stored_events.emplace_back(std::string{thread_id},
      ...
      ste = StoredTransitEvent{std::string{thread_id}, std::string{thread_name},
                               std::move(transit_event)};
  ...
  /***/
  void
  process(std::function<void(TransitEvent const &, std::string_view thread_id,
                             std::string_view thread_name)> const &callback) {
    ...
    for (uint32_t i = 0; i < _stored_events.size(); ++i) {
      ...
      callback(_stored_events[index].transit_event,
               _stored_events[index].thread_id,
...
private:
  struct StoredTransitEvent {
    StoredTransitEvent(std::string thread_id, std::string thread_name,
                       TransitEvent transit_event)
        : thread_id(std::move(thread_id)), thread_name(std::move(thread_name)),
          transit_event(std::move(transit_event)) {}

    /**
     * We use this to take o copy of some objects that are out of space when a
     * thread finishes but the transit events are still in the buffer
     */
