...
class RdtscClock {
  ...
  private:
    ...
    RdtscTicks() {
      ...
      constexpr std::chrono::milliseconds spin_duration =
      ...
      for (size_t i = 0; i < trials; ++i) {
        auto const beg_ts = std::chrono::nanoseconds{
        ...
        uint64_t end_tsc;
        do {
          auto const end_ts = std::chrono::nanoseconds{
              std::chrono::steady_clock::now().time_since_epoch().count()};
          end_tsc = rdtsc();
        ...
        rates[i] = static_cast<double>(end_tsc - beg_tsc) /
                   static_cast<double>(elapsed_ns.count());
      }
...
public:
  ...
  explicit RdtscClock(std::chrono::nanoseconds resync_interval)
      : _resync_interval_ticks(static_cast<std::int64_t>(
            static_cast<double>(resync_interval.count()) *
  ...
  uint64_t time_since_epoch(uint64_t rdtsc_value) const noexcept {
    ...
    // get the current index, this is only sef called my the thread that is
    // doing the resync
    auto const index =
        _version.load(std::memory_order_relaxed) & (_base.size() - 1);

    // get rdtsc current value and compare the diff then add it to base wall
    // time
    auto diff = static_cast<int64_t>(rdtsc_value - _base[index].base_tsc);

    // we need to sync after we calculated otherwise base_tsc value will be
    // ahead of passed tsc value
  ...
  uint64_t time_since_epoch_safe(uint64_t rdtsc_value) const noexcept {
    ...
    // this function won't resync as it can be called by anyone and only a
    // single thread resyncs
    uint32_t version;
    uint64_t wall_ts;
    ...
    do {
      ...
      if (QUILL_UNLIKELY((_base[index].base_tsc) == 0 &&
                         (_base[index].base_time == 0))) {
        return 0;
      }
      ...
      // time
      auto const diff =
          static_cast<int64_t>(rdtsc_value - _base[index].base_tsc);
      wall_ts = static_cast<uint64_t>(
          _base[index].base_time +
          static_cast<int64_t>(static_cast<double>(diff) * _ns_per_tick));
  ...
  bool resync(uint32_t lag) const noexcept {
    // Sometimes we might get an interrupt and might never resync, so we will
    // try again up to max_attempts
    constexpr uint8_t max_attempts{4};

    for (uint8_t attempt = 0; attempt < max_attempts; ++attempt) {
      uint64_t const beg = rdtsc();
      // we force convert to nanoseconds because the precision of
      // system_clock::time-point is not portable across platforms.
      ...
      if (QUILL_LIKELY(end - beg <= lag)) {
        ...
        auto const index =
            (_version.load(std::memory_order_relaxed) + 1) & (_base.size() - 1);
    ...
    // we failed to return earlier and we never resynced, but we don't really
    // want to keep retrying on each call to time_since_epoch() so we do non
    // accurate resync we will increase the resync duration to resync later
...
private:
  struct BaseTimeTsc {
    BaseTimeTsc() = default;
    int64_t base_time{
        0}; /**< Get the initial base time in nanoseconds from epoch */
    uint64_t base_tsc{0}; /**< Get the initial base tsc time */
  };

  /***/
  QUILL_NODISCARD static uint64_t _fast_average(uint64_t x,
                                                uint64_t y) noexcept {
    return (x & y) + ((x ^ y) >> 1);
  }
...
private:
  ...
  int64_t _resync_interval_original{
      0}; /**< stores the initial interval value as as if we fail to resync we
             increase the timer */
  double _ns_per_tick{0};
