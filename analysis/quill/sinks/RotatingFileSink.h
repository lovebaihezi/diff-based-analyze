...
public:
   ...
   */
  enum class RotationFrequency : uint8_t { Disabled, Daily, Hourly, Minutely };

  /**
   * @brief The naming scheme for rotated log files
   */
  enum class RotationNamingScheme : uint8_t { Index, Date, DateAndTime };
   ...
   * @param frequency The frequency of file rotation ('M' for minutes, 'H' for
   * hours)
   * @param interval The rotation interval
   */
  QUILL_ATTRIBUTE_COLD void
  set_rotation_frequency_and_interval(char frequency, uint32_t interval) {
    if (frequency == 'M' || frequency == 'm') {
      ...
      QUILL_THROW(QuillError{"Invalid frequency. Valid values are 'M' or 'm' "
                             "for minutes or 'H' or 'h' for hours"});
    }
   ...
   * @param daily_rotation_time_str The time of day for rotation (format:
   * "HH:MM")
   */
  QUILL_ATTRIBUTE_COLD void
  set_rotation_time_daily(std::string const &daily_rotation_time_str) {
  ...
  QUILL_ATTRIBUTE_COLD void set_max_backup_files(uint32_t value) {
    _max_backup_files = value;
  }

  /**
   * @brief Sets whether the oldest rolled logs should be overwritten when the
   * maximum backup count is reached. If set to false, the oldest logs will not
   * be overwritten when the maximum backup count is reached, and log file
   * rotation will stop. The default value is true.
  ...
  /**
   * @brief Sets whether previous rotated log files should be removed on process
   * start up.
   * @note This option works only when using the mode="w"
   * This is useful to avoid conflicting file names when the process restarts
   * and FilenameAppend::DateTime was not set. The default value is true.
   * @param value True to remove old log files, false otherwise.
   */
  QUILL_ATTRIBUTE_COLD void set_remove_old_files(bool value) {
    _remove_old_files = value;
  }
  ...
  QUILL_NODISCARD std::pair<std::chrono::hours, std::chrono::minutes>
  daily_rotation_time() const noexcept {
...
private:
  /***/
  static std::pair<std::chrono::hours, std::chrono::minutes>
  _disabled_daily_rotation_time() noexcept {
    return std::make_pair(
        std::chrono::hours{std::numeric_limits<std::chrono::hours::rep>::max()},
        std::chrono::minutes{
  ...
  /***/
  static std::pair<std::chrono::hours, std::chrono::minutes>
  _parse_daily_rotation_time(std::string const &daily_rotation_time_str) {
    std::vector<std::string> tokens;
    std::string token;
    size_t start = 0, end = 0;

    while ((end = daily_rotation_time_str.find(':', start)) !=
           std::string::npos) {
    ...
    if (tokens.size() != 2) {
      QUILL_THROW(QuillError{"Invalid daily_rotation_time_str value format. "
    ...
    for (auto const &parsed_token : tokens) {
      if (parsed_token.size() != 2) {
        QUILL_THROW(QuillError{"Invalid daily_rotation_time_str value format. "
                               "Each component of the time (HH and MM) "
                               "should be two digits."});
      }
    }

    auto const daily_rotation_time_str_tp =
        std::make_pair(std::chrono::hours{std::stoi(tokens[0])},
    ...
    if ((daily_rotation_time_str_tp.first > std::chrono::hours{23}) ||
      ...
      QUILL_THROW(QuillError("Invalid rotation values. The hour value should "
                             "be between 00 and 23, and the "
...
private:
  ...
  uint32_t _max_backup_files{
      std::numeric_limits<uint32_t>::max()}; // max means disabled
...
public:
  ...
  RotatingFileSink(fs::path const &filename,
      ...
      : FileSink(filename, static_cast<FileSinkConfig const &>(config),
                 std::move(file_event_notifier), false),
        _config(config) {
    ...
    if (_config.rotation_frequency() !=
        RotatingFileSinkConfig::RotationFrequency::Disabled) {
   ...
   * @param named_args Structured key-value pairs associated with the log
   * message
...
private:
  ...
  QUILL_NODISCARD bool _time_rotation(uint64_t record_timestamp_ns) {
    if (record_timestamp_ns >= _next_rotation_time) {
      ...
      _next_rotation_time =
          _calculate_rotation_tp(record_timestamp_ns, _config);
      return true;
    }
  ...
  void _rotate_files(uint64_t record_timestamp_ns) {
    if ((_created_files.size() > _config.max_backup_files()) &&
        !_config.overwrite_rolled_files()) {
      // We have reached the max number of backup files, and we are not allowed
      // to overwrite the oldest file. We will stop rotating
      return;
    }

    // We need to flush and also fsync before actually getting the size of the
    // file
    FileSink::flush_sink();
    ...
    if (_config.rotation_naming_scheme() ==
        RotatingFileSinkConfig::RotationNamingScheme::Date) {
      datetime_suffix = format_datetime_string(_open_file_timestamp,
                                               _config.timezone(), "%Y%m%d");
    } else if (_config.rotation_naming_scheme() ==
               RotatingFileSinkConfig::RotationNamingScheme::DateAndTime) {
      datetime_suffix = format_datetime_string(
          _open_file_timestamp, _config.timezone(), "%Y%m%d_%H%M%S");
    }
    ...
    for (auto it = _created_files.rbegin(); it != _created_files.rend(); ++it) {
      ...
      existing_file =
          _get_filename(it->base_filename, it->index, it->date_time);

      // increment the index if needed and rename the file
      uint32_t index_to_use = it->index;

      if (_config.rotation_naming_scheme() ==
              RotatingFileSinkConfig::RotationNamingScheme::Index ||
          it->date_time == datetime_suffix) {
        // we are rotating and incrementing the index, or we have another file
        // with the same date_time suffix
        index_to_use += 1;

        renamed_file =
            _get_filename(it->base_filename, index_to_use, datetime_suffix);
    ...
    if (_created_files.size() > _config.max_backup_files()) {
      ...
      fs::path const removed_file = _get_filename(
          _created_files.back().base_filename, _created_files.back().index,
          _created_files.back().date_time);
  ...
  /***/
  void _clean_and_recover_files(fs::path const &filename,
                                std::string const &open_mode,
                                uint64_t today_timestamp_ns) {
    if ((_config.rotation_naming_scheme() !=
         RotatingFileSinkConfig::RotationNamingScheme::Index) &&
        (_config.rotation_naming_scheme() !=
         RotatingFileSinkConfig::RotationNamingScheme::Date)) {
      // clean and recover is only supported for index and date naming scheme,
      // when using DateAndTime there are no collisions in the filenames
      return;
    }

    // if we are starting in "w" mode, then we also should clean all previous
    // log files of the previous run
    if (_config.remove_old_files() && (open_mode == "w")) {
      for (const auto &entry : fs::directory_iterator(fs::current_path() /
        ...
        if (entry.path().filename().string().find(filename.stem().string() +
                                                  ".") != 0) {
          // expect to find filename.stem().string() exactly at the start of the
          // filename
          continue;
        }
        ...
        if (_config.rotation_naming_scheme() ==
          ...
          if (size_t const pos = entry.path().stem().string().find_last_of('.');
              pos != std::string::npos) {
            // Get the today's date, we won't remove the files of the previous
            // dates as they won't collide
            std::string const today_date = format_datetime_string(
                today_timestamp_ns, _config.timezone(), "%Y%m%d");

            if (std::string const index_or_date =
                    entry.path().stem().string().substr(
                        pos + 1, entry.path().stem().string().length());
                (index_or_date.length() >= 8) &&
                (index_or_date == today_date)) {
              ...
              std::string const filename_with_date =
                  entry.path().filename().string().substr(0, pos);

              if (size_t const second_last =
                      filename_with_date.find_last_of('.');
                  second_last != std::string::npos) {
                if (std::string const date_part = filename_with_date.substr(
                        second_last + 1, filename_with_date.length());
      ...
      for (const auto &entry : fs::directory_iterator(fs::current_path() /
        ...
        std::string const extension =
            entry.path().extension().string(); // e.g. ".log"
        ...
        if (size_t const pos = entry.path().stem().string().find_last_of('.');
          ...
          if (_config.rotation_naming_scheme() ==
              RotatingFileSinkConfig::RotationNamingScheme::Index) {
            std::string const index = entry.path().stem().string().substr(
                pos + 1, entry.path().stem().string().length());

            std::string const current_filename =
                entry.path().filename().string().substr(0, pos) + extension;
            ...
            QUILL_TRY {
              _created_files.emplace_front(
                  current_file, static_cast<uint32_t>(std::stoul(index)),
                  std::string{});
            }
            ...
            if (std::string const index_or_date =
              ...
              if (size_t const second_last =
                ...
                if (std::string const date_part = filename_with_date.substr(
                    ...
                    date_part == today_date) {
                  std::string const current_filename =
                      filename_with_date.substr(0, second_last) + extension;
                  fs::path current_file = entry.path().parent_path();
                  current_file.append(current_filename);
                  ...
                  QUILL_TRY {
                    _created_files.emplace_front(
                        current_file,
                        static_cast<uint32_t>(std::stoul(index_or_date)),
                        date_part);
                  }
                  QUILL_CATCH_ALL() { continue; }
                }
  ...
  /***/
  bool static _rename_file(fs::path const &previous_file,
                           fs::path const &new_file) noexcept {
    std::error_code ec;
  ...
  /***/
  QUILL_NODISCARD static fs::path
  _append_index_to_filename(fs::path const &filename, uint32_t index) noexcept {
    if (index == 0u) {
      return filename;
    }
  ...
  /***/
  QUILL_NODISCARD static fs::path
  _append_string_to_filename(fs::path const &filename,
  ...
  /***/
  static uint64_t
  _calculate_initial_rotation_tp(uint64_t start_time_ns,
                                 RotatingFileSinkConfig const &config) {
    time_t const time_now = static_cast<time_t>(start_time_ns) / 1000000000;
    tm date;

    // here we do this because of `daily_rotation_time_str` that might have
    // specified the time in UTC
    ...
    if (config.rotation_frequency() ==
        RotatingFileSinkConfig::RotationFrequency::Minutely) {
      date.tm_min += 1;
      date.tm_sec = 0;
    } else if (config.rotation_frequency() ==
               RotatingFileSinkConfig::RotationFrequency::Hourly) {
      date.tm_hour += 1;
      date.tm_min = 0;
      date.tm_sec = 0;
    } else if (config.rotation_frequency() ==
               RotatingFileSinkConfig::RotationFrequency::Daily) {
      date.tm_hour = static_cast<decltype(date.tm_hour)>(
          config.daily_rotation_time().first.count());
      date.tm_min = static_cast<decltype(date.tm_min)>(
    ...
    time_t const rotation_time = (config.timezone() == Timezone::GmtTime)
                                     ? detail::timegm(&date)
                                     : std::mktime(&date);

    uint64_t const rotation_time_seconds =
    ...
    return static_cast<uint64_t>(std::chrono::nanoseconds{
        std::chrono::seconds{
            rotation_time_seconds}}.count());
  }

  /***/
  static uint64_t _calculate_rotation_tp(uint64_t rotation_timestamp_ns,
                                         RotatingFileSinkConfig const &config) {
    if (config.rotation_frequency() ==
        RotatingFileSinkConfig::RotationFrequency::Minutely) {
      return rotation_timestamp_ns +
             static_cast<uint64_t>(std::chrono::nanoseconds{
                 std::chrono::minutes{config.rotation_interval()}}
                                       .count());
    }
    ...
    if (config.rotation_frequency() ==
      ...
      return rotation_timestamp_ns +
             static_cast<uint64_t>(std::chrono::nanoseconds{
                 std::chrono::hours{config.rotation_interval()}}
                                       .count());
    }
    ...
    if (config.rotation_frequency() ==
        RotatingFileSinkConfig::RotationFrequency::Daily) {
      return rotation_timestamp_ns +
             std::chrono::nanoseconds{std::chrono::hours{24}}.count();
    }
  ...
  /***/
  static fs::path _get_filename(fs::path filename, uint32_t index,
...
private:
  struct FileInfo {
    FileInfo(fs::path base_filename, uint32_t index, std::string date_time)
        : base_filename{std::move(base_filename)},
          date_time{std::move(date_time)}, index{index} {}
  ...
  std::deque<FileInfo>
      _created_files; /**< We store in a queue the filenames we created, first:
                         index, second: date/datetime, third: base_filename */
  uint64_t _next_rotation_time; /**< The next rotation time point */
  uint64_t _open_file_timestamp{
      0};               /**< The timestamp of the currently open file */
