...
public:
   ...
   * When this option is set, the file name will be appended with the start date
   * or date and time timestamp of when the process started.
   *
   * For example:
   * application.log -> application_20230101.log  (StartDate)
   * application.log -> application_20230101_121020.log  (StartDateTime)
   *
   * @param value The append type to set. Valid options are Date and
   * DateAndTime.
   * @param append_filename_format_pattern Specifies a custom `strftime` format
   * pattern to use for the filename. This parameter is only applicable when
   * `FilenameAppendOption::CustomDateTimeFormat` is selected
   */
  QUILL_ATTRIBUTE_COLD void set_filename_append_option(
    ...
    if (_filename_append_option ==
        FilenameAppendOption::StartCustomTimestampFormat) {
      if (append_filename_format_pattern.empty()) {
        QUILL_THROW(QuillError{"The 'CustomDateTimeFormat' option was "
                               "specified, but no format pattern was provided. "
  ...
  /**
   * @brief Sets the timezone to use for time-based operations e.g. when
   * appending the date to the get_filename or when setting the logging pattern.
   ...
   *
   * This function allows you to specify a custom buffer size for fwrite,
   * improving efficiency for file write operations.
   *
   * To disable custom buffering and revert to the default size, pass a value of
   * 0.
   *
   * @note By default, a buffer size of 64 KB is used.
   * @param value Size of the buffer in bytes. If set to 0, the default buffer
   * size will be used.
   */
  ...
  /**
   * Sets the minimum interval between `fsync` calls. This specifies the minimum
   * time between consecutive `fsync` operations but does not guarantee that
   * `fsync` will be called exactly at that interval.
   *
   * For example, if some messages are flushed to the log and `fsync` is skipped
   * because it was previously called, and no further messages are written to
   * the file, `fsync` will not be called even if the minimum interval has
   * passed. This is because the previous call was skipped due to the interval,
   * and no new messages necessitate another `fsync` call.
   *
   * This feature is intended to mitigate concerns about frequent `fsync` calls
   * potentially causing disk wear.
   *
   * Note: This option is only applicable when `fsync` is enabled. By default,
   * the value is 0, which means that `fsync` will be called periodically by the
   * backend worker thread when messages are written to the file, irrespective
   * of the interval.
   *
   * @param value The minimum interval, in milliseconds, between `fsync` calls.
   */
  QUILL_ATTRIBUTE_COLD void
  set_minimum_fsync_interval(std::chrono::milliseconds value) {
  ...
  QUILL_NODISCARD std::string const &
  append_filename_format_pattern() const noexcept {
  ...
  QUILL_NODISCARD std::chrono::milliseconds
  minimum_fsync_interval() const noexcept {
...
public:
  ...
  explicit FileSink(fs::path const &filename,
      ...
      : StreamSink(_get_updated_filename_with_appended_datetime(
                       filename, config.filename_append_option(),
    ...
    if (!_config.fsync_enabled() &&
        (_config.minimum_fsync_interval().count() != 0)) {
      QUILL_THROW(QuillError{"Cannot set a non-zero minimum fsync interval "
  ...
  QUILL_ATTRIBUTE_HOT void flush_sink() override {
    if (!_write_occurred || !_file) {
      // Check here because StreamSink::flush() will set _write_occurred to
      // false
      return;
    }
    ...
    if (!fs::exists(_filename)) {
      // after flushing the file we can check if the file still exists. If not
      // we reopen it. This can happen if a user deletes a file while the
...
protected:
   ...
   */
  QUILL_NODISCARD static std::string
  format_datetime_string(uint64_t timestamp_ns, Timezone time_zone,
   ...
   */
  QUILL_NODISCARD static std::pair<std::string, std::string>
  extract_stem_and_extension(fs::path const &filename) noexcept {
    // filename and extension
    return std::make_pair((filename.parent_path() / filename.stem()).string(),
  ...
  QUILL_NODISCARD static fs::path append_datetime_to_filename(
    ...
    return stem +
           format_datetime_string(timestamp_ns, time_zone,
                                  append_filename_format_pattern) +
           ext;
  }
  ...
  void open_file(fs::path const &filename, std::string const &mode) {
    ...
    if (!_file) {
      QUILL_THROW(QuillError{std::string{"fopen failed failed path: "} +
                             filename.string() + " mode: " + mode +
    ...
    if (_config.write_buffer_size() != 0) {
      ...
      if (setvbuf(_file, _write_buffer.get(), _IOFBF,
                  _config.write_buffer_size()) != 0) {
        QUILL_THROW(QuillError{std::string{"setvbuf failed error: "} +
                               strerror(errno)});
      }
    }
...
private:
  ...
  QUILL_NODISCARD static fs::path _get_updated_filename_with_appended_datetime(
    ...
    if ((append_to_filename_option == FilenameAppendOption::None) ||
        (filename == "/dev/null")) {
      return filename;
    }

    if ((append_to_filename_option ==
         FilenameAppendOption::StartCustomTimestampFormat) ||
        (append_to_filename_option == FilenameAppendOption::StartDate) ||
        (append_to_filename_option == FilenameAppendOption::StartDateTime)) {
      return append_datetime_to_filename(
          filename, append_filename_format_pattern, time_zone, timestamp);
    }
