...
/**
 * @brief Notifies on file events by calling the appropriate callback, the
 * callback is executed on the backend worker thread
 */
...
public:
   ...
   */
  explicit StreamSink(
      fs::path stream, FILE *file = nullptr,
    ...
    if (_filename == std::string{"stdout"}) {
      ...
      if (!_filename.parent_path().empty()) {
        ...
        // The call to fs::status is necessary due to a known issue in GCC
        // versions 8.3.0 to 9.4.0. In these versions,
        // fs::create_directories(path, ec) internally uses
        // fs::symlink_status(path, ec) instead of fs::status(path, ec) for
        // checking the path. This causes a problem because fs::symlink_status
        // does not follow the symlink to the target directory. As a result,  it
        // fails the is_directory() check but still indicates that the path
        // exists, leading to a not_a_directory exception being set in the error
        // code
        ...
        if (!is_directory(st)) {
          ...
          if (ec) {
            ...
            QUILL_THROW(
                QuillError{std::string{"cannot create directories for path "} +
                           parent_path.string() + std::string{" - error: "} +
                           ec.message()});
          }
        }
      } else {
      ...
      if (ec) {
        ...
        QUILL_THROW(QuillError{
            std::string{"cannot create canonical path for path "} +
            parent_path.string() + std::string{" - error: "} + ec.message()});
      }

      // finally replace the given filename's parent_path with the equivalent
      // canonical path
  ...
  QUILL_ATTRIBUTE_HOT void write_log(
    ...
    if (QUILL_UNLIKELY(!_file)) {
      // FileSink::flush() tries to re-open a deleted file and if it fails _file
      // can be null
      return;
    }

    if (_file_event_notifier.before_write) {
      std::string const user_log_statement =
          _file_event_notifier.before_write(log_statement);

      safe_fwrite(user_log_statement.data(), sizeof(char),
                  user_log_statement.size(), _file);
    } else {
      safe_fwrite(log_statement.data(), sizeof(char), log_statement.size(),
                  _file);
    }
   ...
   */
  QUILL_NODISCARD virtual fs::path const &get_filename() const noexcept {
    return _filename;
  }
...
protected:
   ...
   */
  QUILL_ATTRIBUTE_HOT static void safe_fwrite(void const *ptr, size_t size,
                                              size_t count, FILE *stream) {
    size_t const written = std::fwrite(ptr, size, count, stream);

    if (QUILL_UNLIKELY(written < count)) {
      QUILL_THROW(QuillError{std::string{"fwrite failed errno: "} +
                             std::to_string(errno) +
