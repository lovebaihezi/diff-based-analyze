 ...
 *
 * This class facilitates the asynchronous logging of CSV files, where
 * formatting and I/O operations are handled by the backend worker thread.
 *
 * @tparam TCsvSchema A user-defined struct specifying the CSV schema at
 * compile-time.
 * @tparam TFrontendOptions Custom frontend_t options if they are used
 * application-wide. If no custom frontend_t options are used, then use
 * quill::frontend_tOptions.
 *
 ...
 * "order_id,symbol,quantity,price,side"; static constexpr char const* format =
 * "{},{},{},{:.2f},{}";
 * };
 * @endcode
 */
template <typename TCsvSchema, typename TFrontendOptions> class CsvWriter {
public:
   ...
   * @param open_mode The mode in which to open the file ('w' for write, 'a' for
   * append).
   * @param filename_append Option to append to the filename (None, Date,
   * DateTime).
   */
  explicit CsvWriter(
      std::string const &filename, char open_mode = 'w',
    ...
    _logger = frontend_t::create_or_get_logger(
        ...
        frontend_t::template create_or_get_sink<FileSink>(
            filename,
    ...
    _logger->template log_statement<false, false>(
        LogLevel::None, &_header_metadata, TCsvSchema::header);
  }
   ...
   * @param sink The sink to output the data to (e.g., a ConsoleSink or a
   * user-defined Sink).
   */
  CsvWriter(std::string const &unique_name, std::shared_ptr<Sink> sink) {
    _logger = frontend_t::create_or_get_logger(
        _logger_name_prefix + unique_name, std::move(sink),
        PatternFormatterOptions{"%(message)", "", Timezone::GmtTime});
   ...
   */
  CsvWriter(std::string const &unique_name,
            std::initializer_list<std::shared_ptr<Sink>> sinks) {
    _logger = frontend_t::create_or_get_logger(
        _logger_name_prefix + unique_name, sinks,
        PatternFormatterOptions{"%(message)", "", Timezone::GmtTime});
   ...
   */
  template <typename... Args> void append_row(Args &&...fields) {
    _logger->template log_statement<false, false>(LogLevel::None,
                                                  &_line_metadata, fields...);
  }

  /**
   * @brief Flushes the log to ensure all data is written to the file.
   * This method will block the caller thread until the file is flushed,
   * ensuring that all data are flushed to the file
   */
