...
struct rgb {
  ...
  FMTQUILL_CONSTEXPR rgb(uint8_t r_, uint8_t g_, uint8_t b_)
      : r(r_), g(g_), b(b_) {}
...
struct color_type {
  ...
  FMTQUILL_CONSTEXPR color_type(color rgb_color) noexcept
      : is_rgb(true), value{} {
    value.rgb_color = static_cast<uint32_t>(rgb_color);
  }
  FMTQUILL_CONSTEXPR color_type(rgb rgb_color) noexcept
      : is_rgb(true), value{} {
...
public:
  ...
  operator|(text_style lhs, const text_style &rhs) -> text_style {
    return lhs |= rhs;
  }
  ...
  get_foreground() const noexcept -> detail::color_type {
    FMTQUILL_ASSERT(has_foreground(), "no foreground specified for this style");
    return foreground_color;
  }
  FMTQUILL_CONSTEXPR auto
  get_background() const noexcept -> detail::color_type {
...
private:
  ...
  friend FMTQUILL_CONSTEXPR auto
  fg(detail::color_type foreground) noexcept -> text_style;

  friend FMTQUILL_CONSTEXPR auto
  bg(detail::color_type background) noexcept -> text_style;
...
FMTQUILL_CONSTEXPR inline auto operator|(emphasis lhs,
...
template <typename Char> struct ansi_color_escape {
  ...
  FMTQUILL_CONSTEXPR auto begin() const noexcept -> const Char * {
    return buffer;
  }
  FMTQUILL_CONSTEXPR20 auto end() const noexcept -> const Char * {
...
private:
  ...
  static FMTQUILL_CONSTEXPR auto has_emphasis(emphasis em,
...
FMTQUILL_CONSTEXPR auto make_foreground_color(
    detail::color_type foreground) noexcept -> ansi_color_escape<Char> {
  return ansi_color_escape<Char>(foreground, "\x1b[38;2;");
}

template <typename Char>
FMTQUILL_CONSTEXPR auto make_background_color(
...
template <typename Char>
FMTQUILL_CONSTEXPR auto
make_emphasis(emphasis em) noexcept -> ansi_color_escape<Char> {
...
inline void vprint(FILE *f, const text_style &ts, string_view fmt,
  ...
  print(f, FMTQUILL_STRING("{}"), string_view(buf.begin(), buf.size()));
}
...
void print(FILE *f, const text_style &ts, format_string<T...> fmt,
           T &&...args) {
  vprint(f, ts, fmt, fmtquill::make_format_args(args...));
}
 ...
 * ```
 * #include <fmt/color.h>
 * std::string message = fmtquill::format(fmtquill::emphasis::bold |
 * fg(fmtquill::color::red), "The answer is {}", 42);
 * ```
 */
template <typename... T>
inline auto format(const text_style &ts, format_string<T...> fmt,
                   T &&...args) -> std::string {
  return fmtquill::vformat(ts, fmt, fmtquill::make_format_args(args...));
}
 ...
 *                    fmtquill::emphasis::bold | fg(fmtquill::color::red), "{}",
 * 42);
 */
...
inline auto format_to(OutputIt out, const text_style &ts,
  ...
  return vformat_to(out, ts, fmt, fmtquill::make_format_args(args...));
}
...
template <typename T>
FMTQUILL_CONSTEXPR auto
styled(const T &value, text_style ts) -> detail::styled_arg<remove_cvref_t<T>> {
