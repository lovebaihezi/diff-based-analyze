...
auto range_begin(T &&rng)
    FMTQUILL_DECLTYPE_RETURN(static_cast<T &&>(rng).begin());
template <typename T>
...
template <typename ParseContext> struct parse_empty_specs {
  template <typename Formatter>
  FMTQUILL_CONSTEXPR void operator()(Formatter &f) {
    f.parse(ctx);
...
template <typename Context> struct range_mapper {
  ...
  template <typename T, FMTQUILL_ENABLE_IF(
                            has_formatter<remove_cvref_t<T>, Context>::value)>
  ...
  template <typename T, FMTQUILL_ENABLE_IF(
                            !has_formatter<remove_cvref_t<T>, Context>::value)>
  static auto
  map(T &&value) -> decltype(mapper().map(static_cast<T &&>(value))) {
...
private:
  ...
  template <typename Output, typename It, typename Sentinel, typename U = T,
            FMTQUILL_ENABLE_IF(std::is_same<U, Char>::value)>
...
public:
  ...
  underlying() -> detail::range_formatter_type<Char, T> & {
    return underlying_;
  }
 ...
 *     fmtquill::print("{}", fmtquill::join(v, ", "));
 *     // Output: 1, 2, 3
 *
 * `fmtquill::join` applies passed format specifiers to the range elements:
 *
 *     fmtquill::print("{:02}", fmtquill::join(v, ", "));
 *     // Output: 01, 02, 03
 */
template <typename Range>
auto join(Range &&r,
...
// Define FMTQUILL_TUPLE_JOIN_SPECIFIERS to enable experimental format
// specifiers support in tuple_join. It is disabled by default because of issues
// with the dynamic width and precision.
