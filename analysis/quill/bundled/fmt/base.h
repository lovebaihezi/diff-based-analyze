...
#elif FMTQUILL_HAS_FEATURE(cxx_relaxed_constexpr) ||                           \
    FMTQUILL_MSC_VERSION >= 1912
#define FMTQUILL_USE_CONSTEXPR 1
#else
...
#if FMTQUILL_HAS_CPP_ATTRIBUTE(noreturn) && !FMTQUILL_MSC_VERSION &&           \
    !defined(__NVCC__)
...
#if defined(__GXX_RTTI) || FMTQUILL_HAS_FEATURE(cxx_rtti) ||                   \
...
FMTQUILL_CONSTEXPR auto length(const Char *s) -> size_t {
  size_t len = 0;
  while (*s++)
    ++len;
  return len;
...
template <typename Char>
FMTQUILL_CONSTEXPR auto compare(const Char *s1, const Char *s2,
                                std::size_t n) -> int {
 ...
 * subset of the API. `fmtquill::basic_string_view` is used for format strings
...
public:
  ...
  template <typename S, FMTQUILL_ENABLE_IF(
...
// Constructs fmtquill::basic_string_view<Char> from types implicitly
// convertible to it, deducing Char. Explicitly convertible types such as the
// ones returned from FMTQUILL_STRING are intentionally excluded.
...
constexpr auto
to_string_view(const T &s) -> basic_string_view<typename T::value_type> {
  return s;
}
template <typename Char>
constexpr auto
to_string_view(basic_string_view<Char> s) -> basic_string_view<Char> {
  return s;
}
...
FMTQUILL_DEPRECATED FMTQUILL_NORETURN inline void
throw_format_error(const char *message) {
...
public:
  ...
  iterator_buffer(iterator_buffer &&other) noexcept
      : Traits(other), buffer<T>(grow, data_, 0, buffer_size),
        out_(other.out_) {}
  ~iterator_buffer() {
...
private:
  ...
  static FMTQUILL_CONSTEXPR void grow(buffer<value_type> &buf,
                                      size_t capacity) {
...
basic_format_parse_context<Char>::do_check_arg_id(int id) {
...
template <typename T, typename InputIt, typename OutputIt,
          FMTQUILL_ENABLE_IF(!is_back_insert_iterator<OutputIt>::value)>
FMTQUILL_CONSTEXPR auto copy(InputIt begin, InputIt end,
...
public:
  ...
  constexpr FMTQUILL_ALWAYS_INLINE value(long long val)
      : long_long_value(val) {}
  ...
  FMTQUILL_CONSTEXPR FMTQUILL_ALWAYS_INLINE
  value(basic_string_view<char_type> val) {
    string.data = val.data();
    string.size = val.size();
  }
  FMTQUILL_ALWAYS_INLINE value(const void *val) : pointer(val) {}
  FMTQUILL_ALWAYS_INLINE value(const named_arg_info<char_type> *args,
                               size_t size)
      : named_args{args, size} {}

  template <typename T>
  FMTQUILL_CONSTEXPR20 FMTQUILL_ALWAYS_INLINE value(T &val) {
...
template <typename T> struct format_as_result {
  template <typename U, FMTQUILL_ENABLE_IF(std::is_enum<U>::value ||
...
template <typename Context> struct arg_mapper {
  ...
  FMTQUILL_MAP_API auto map(const char_type *val) -> const char_type * {
    return val;
  }
  ...
  FMTQUILL_MAP_API auto
  map(const T &val) -> decltype(FMTQUILL_DECLTYPE_THIS map(U())) {
  ...
  FMTQUILL_MAP_API auto
  map(T &val) -> decltype(FMTQUILL_DECLTYPE_THIS do_map(val)) {
    return do_map(val);
  }
...
template <typename Char, typename T,
          FMTQUILL_ENABLE_IF(!is_named_arg<T>::value)>
...
// `fmtquill::basic_format_args` for passing into type-erased formatting
// functions such as `fmtquill::vformat`.
...
private:
  ...
  friend FMTQUILL_CONSTEXPR auto
  detail::make_arg(T &value) -> basic_format_arg<ContextType>;
...
public:
  ...
  visit(Visitor &&vis) const -> decltype(vis(0)) {
    switch (type_) {
...
FMTQUILL_DEPRECATED FMTQUILL_CONSTEXPR auto
visit_format_arg(Visitor &&vis,
 ...
 *     fmtquill::format_args args = fmtquill::make_format_args();  // Dangling
 * reference
 */
...
public:
  ...
  constexpr FMTQUILL_ALWAYS_INLINE
  basic_format_args(const detail::format_arg_store<Context, NUM_ARGS,
                                                   NUM_NAMED_ARGS, DESC> &store)
...
#endif
namespace align {
enum type FMTQUILL_ENUM_UNDERLYING_TYPE(unsigned char) {
  none,
  left,
  right,
  center,
...
struct format_specs {
  ...
  constexpr format_specs()
      : width(0), precision(-1), type(presentation_type::none),
        align(align::none), sign(sign::none), upper(false), alt(false),
        localized(false) {}
};
...
template <typename Char>
FMTQUILL_CONSTEXPR auto parse_nonnegative_int(const Char *&begin,
                                              const Char *end,
...
template <typename Char>
FMTQUILL_CONSTEXPR auto
parse_dynamic_spec(const Char *begin, const Char *end, int &value,
                   arg_ref<Char> &ref,
...
template <typename Char>
FMTQUILL_CONSTEXPR auto
parse_precision(const Char *begin, const Char *end, int &value,
                arg_ref<Char> &ref,
                basic_format_parse_context<Char> &ctx) -> const Char * {
  ++begin;
  if (begin == end || *begin == '}') {
...
template <typename Char>
FMTQUILL_CONSTEXPR auto parse_format_specs(
  ...
  struct {
    ...
    FMTQUILL_CONSTEXPR auto operator()(pres pres_type,
                                       int set) -> const Char * {
      if (!in(arg_type, set)) {
...
template <typename Char, typename Handler>
FMTQUILL_CONSTEXPR auto
parse_replacement_field(const Char *begin, const Char *end,
                        Handler &&handler) -> const Char * {
  struct id_adapter {
    Handler &handler;
    int arg_id;
...
check_char_specs(const format_specs &specs) -> bool {
...
FMTQUILL_CONSTEXPR auto
get_arg_index_by_name(basic_string_view<Char> name) -> int {
#if FMTQUILL_USE_NONTYPE_TEMPLATE_ARGS
  if constexpr (sizeof...(Args) > 0)
...
public:
  ...
  FMTQUILL_CONSTEXPR auto on_format_specs(int id, const Char *begin,
...
FMTQUILL_API void vprint_mojibake(FILE *, string_view, format_args,
                                  bool = false);
#ifndef _WIN32
...
public:
  ...
  template <type U = TYPE,
            FMTQUILL_ENABLE_IF(U == type::string_type ||
                               U == type::cstring_type || U == type::char_type)>
...
public:
  ...
  FMTQUILL_CONSTEVAL FMTQUILL_ALWAYS_INLINE basic_format_string(const S &s)
      : str_(s) {
    static_assert(
        detail::count<
  ...
  FMTQUILL_ALWAYS_INLINE operator basic_string_view<Char>() const {
    return str_;
  }
...
template <typename OutputIt,
          FMTQUILL_ENABLE_IF(detail::is_output_iterator<
...
template <typename OutputIt, typename... T,
          FMTQUILL_ENABLE_IF(detail::is_output_iterator<
                             remove_cvref_t<OutputIt>, char>::value)>
FMTQUILL_INLINE auto format_to(OutputIt &&out, format_string<T...> fmt,
                               T &&...args) -> remove_cvref_t<OutputIt> {
  return vformat_to(FMTQUILL_FWD(out), fmt,
...
template <typename OutputIt, typename... T,
          FMTQUILL_ENABLE_IF(detail::is_output_iterator<OutputIt, char>::value)>
FMTQUILL_INLINE auto format_to_n(OutputIt out, size_t n,
                                 format_string<T...> fmt,
...
FMTQUILL_INLINE auto format_to(char (&out)[N], format_string<T...> fmt,
