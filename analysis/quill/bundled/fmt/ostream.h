...
inline auto write_ostream_unicode(std::ostream &os,
                                  fmtquill::string_view data) -> bool {
  FILE *f = nullptr;
#if FMTQUILL_MSC_VERSION && FMTQUILL_USE_RTTI
  if (auto *buf = dynamic_cast<std::filebuf *>(os.rdbuf()))
    f = get_file(*buf);
  else
    return false;
#elif defined(_WIN32) && defined(__GLIBCXX__) && FMTQUILL_USE_RTTI
  auto *rdbuf = os.rdbuf();
...
inline auto
write_ostream_unicode(std::wostream &,
...
void print(std::wostream &os,
           ...
           Args &&...args) {
  vprint(os, fmt,
         fmtquill::make_format_args<buffered_context<wchar_t>>(args...));
}
