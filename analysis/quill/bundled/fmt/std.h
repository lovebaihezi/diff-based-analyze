...
public:
  ...
  FMTQUILL_CONSTEXPR auto parse(ParseContext &ctx) {
...
private:
  ...
  template <class U>
  FMTQUILL_CONSTEXPR static auto
  maybe_set_debug_format(U &u, bool set) -> decltype(u.set_debug_format(set)) {
...
template <typename Char> struct formatter<std::error_code, Char> {
  ...
  format(const std::error_code &ec,
         FormatContext &ctx) const -> decltype(ctx.out()) {
    auto out = ctx.out();
...
struct formatter<BitRef, Char,
  ...
  template <typename FormatContext>
  FMTQUILL_CONSTEXPR auto
  format(const BitRef &v, FormatContext &ctx) const -> decltype(ctx.out()) {
...
private:
  ...
  FMTQUILL_CONSTEXPR auto
  do_format(const std::complex<T> &c, detail::dynamic_format_specs<Char> &specs,
            FormatContext &ctx, OutputIt out) const -> OutputIt {
    if (c.real() != 0) {
      *out++ = Char('(');
