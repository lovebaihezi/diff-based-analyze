...
/** Specialization for arrays of arithmetic types and enums, char arrays are
 * handled in Codec.h **/
template <typename T, std::size_t N>
struct Codec<T[N], std::enable_if_t<std::conjunction_v<
...
template <typename T, size_t N> struct Codec<std::array<T, N>> {
  ...
  compute_encoded_size(detail::SizeCacheVector &conditional_arg_size_cache,
    ...
    if constexpr (std::disjunction_v<std::is_arithmetic<T>, std::is_enum<T>>) {
      ...
      // Note: This excludes all trivially copyable types; e.g.,
      // std::string_view should not fall into this branch.
      return sizeof(T) * N;
    } else {
