...
struct Codec<
    ...
    std::enable_if_t<std::disjunction_v<
        std::is_same<T, wchar_t *>, std::is_same<T, wchar_t const *>,
  ...
  compute_encoded_size(detail::SizeCacheVector &conditional_arg_size_cache,
    ...
    // we can retrieve it when we decode. We do not store the null terminator in
    // the buffer
  ...
  static void encode(std::byte *&buffer,
    ...
    if (len != 0) {
      // copy the string, no need to zero terminate it as we got the length and
      // e.g a wstring_view might not always be zero terminated
  ...
  static std::wstring_view decode_arg(std::byte *&buffer) {
    ...
    buffer += sizeof(len);
    auto arg =
        std::wstring_view{reinterpret_cast<wchar_t const *>(buffer), len};
  ...
  static void decode_and_store_arg(std::byte *&buffer,
    ...
    args_store->push_back(detail::utf8_encode(
