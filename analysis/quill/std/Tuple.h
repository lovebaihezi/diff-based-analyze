...
template <typename... Types> struct Codec<std::tuple<Types...>> {
  static size_t
  compute_encoded_size(detail::SizeCacheVector &conditional_arg_size_cache,
    ...
    std::apply(
        [&total_size, &conditional_arg_size_cache](auto const &...elems) {
          ((total_size +=
            Codec<std::decay_t<decltype(elems)>>::compute_encoded_size(
                conditional_arg_size_cache, elems)),
           ...);
        },
        arg);
  ...
  static void encode(std::byte *&buffer,
    ...
    std::apply(
        [&conditional_arg_size_cache, &conditional_arg_size_cache_index,
         &buffer](auto const &...elems) {
          ((Codec<std::decay_t<decltype(elems)>>::encode(
               buffer, conditional_arg_size_cache,
  ...
  static auto decode_arg(std::byte *&buffer) {
    ...
    std::apply(
        [&buffer](auto &...elems) {
          ((elems = Codec<std::decay_t<decltype(elems)>>::decode_arg(buffer)),
           ...);
        },
        arg);
