...
template <> struct Codec<fs::path> {
  static size_t
  compute_encoded_size(detail::SizeCacheVector &conditional_arg_size_cache,
                       fs::path const &arg) noexcept {
    if constexpr (std::is_same_v<fs::path::string_type, std::string>) {
      return Codec<std::string>::compute_encoded_size(
          conditional_arg_size_cache, arg.string());
    }
#if defined(_WIN32)
    else if constexpr (std::is_same_v<fs::path::string_type, std::wstring>) {
      return Codec<std::wstring>::compute_encoded_size(
          conditional_arg_size_cache, arg.wstring());
    }
#endif
  }
  ...
  static void encode(std::byte *&buffer,
    ...
    if constexpr (std::is_same_v<fs::path::string_type, std::string>) {
      Codec<std::string>::encode(buffer, conditional_arg_size_cache,
                                 conditional_arg_size_cache_index,
                                 arg.string());
    }
#if defined(_WIN32)
    else if constexpr (std::is_same_v<fs::path::string_type, std::wstring>) {
