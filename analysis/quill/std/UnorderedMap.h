...
template <template <typename...> class UnorderedMapType, typename Key,
          typename T, typename Hash, typename KeyEqual, typename Allocator>
struct Codec<
    UnorderedMapType<Key, T, Hash, KeyEqual, Allocator>,
    std::enable_if_t<std::disjunction_v<
        std::is_same<UnorderedMapType<Key, T, Hash, KeyEqual, Allocator>,
  ...
  static size_t compute_encoded_size(
    ...
    if constexpr (std::conjunction_v<std::disjunction<std::is_arithmetic<Key>,
                                                      std::is_enum<Key>>,
      ...
      for (auto const &elem : arg) {
        total_size += Codec<std::pair<Key, T>>::compute_encoded_size(
            conditional_arg_size_cache, elem);
      }
    }
  ...
  static auto decode_arg(std::byte *&buffer) {
    ...
    if constexpr (
      ...
      constexpr bool wide_key_t = std::is_same_v<Key, wchar_t *> ||
                                  std::is_same_v<Key, wchar_t const *> ||
                                  std::is_same_v<Key, std::wstring> ||
                                  std::is_same_v<Key, std::wstring_view>;

      constexpr bool wide_value_t = std::is_same_v<T, wchar_t *> ||
