...
export async function checkVulnerbilities(
  model: GenerativeModelPreview,
  code: string,
) {
  const sysPmt = `
You are an Expert in POSIX Thread Model and Linux API, including sync prototype, atomic, CPU cache and memory corruption. You will check the input source code, point the issues including: wrong memory order of atomic variables read and write; dead lock; non-thread safe API uses in thread scope. The input code will use thpool.h:
\`\`\`C
